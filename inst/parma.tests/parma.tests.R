...
parma.test1 <- function() {
    ...
    colnames(pmatrix) <- c("mad", "ev", "sd", "minimax", "cvar", "cdar",
        "lpm[m,t=c]", "lpm[m,t=mu]")
    ...
    pmatrix[1, 1] <- as.integer(riskfun(w, b * Data, risk = "mad")/abs(b) ==
        m1)
    ...
    # scaling property: YES
    pmatrix[1, 2] <- as.integer(riskfun(w, b * Data, risk = "ev")/b^2 ==
        m2)
    # location invariant: YES
    ...
    # scaling property: YES
    pmatrix[1, 3] <- as.integer(sqrt(riskfun(w, b * Data, risk = "ev"))/abs(b) ==
        m3)
    # location invariant: YES
    pmatrix[2, 3] <- as.integer(sqrt(riskfun(w, a + Data, risk = "ev")) ==
        m3)
    # Subadditivity: YES
    ...
    # scaling property: YES*
    pmatrix[1, 4] <- as.integer(riskfun(w, abs(b) * Data, risk = "minimax")/abs(b) ==
        m4)
    # location invariant: NO[1]/YES[2]*
    pmatrix[2, 4] <- as.integer(riskfun(w, a + Data, risk = "minimax") ==
        m4)
    pmatrix[3, 4] <- as.integer(round(riskfun(w, a + Data, risk = "minimax") +
        a, 12) == round(m4, 12))
    # Subadditivity: YES
    ...
    # scaling property: YES*
    pmatrix[1, 5] <- as.integer(round(riskfun(w, abs(b) * Data, risk = "cvar",
        alpha = 0.05)/abs(b), 12) == round(m5, 12))
    # location invariant: NO[1]/YES[2]*
    pmatrix[2, 5] <- as.integer(round(riskfun(w, a + Data, risk = "cvar",
        alpha = 0.05), 12) == round(m5, 12))
    pmatrix[3, 5] <- as.integer(round(riskfun(w, a + Data, risk = "cvar",
        alpha = 0.05) + a, 12) == round(m5, 12))
    # Subadditivity: YES
    pmatrix[4, 5] <- as.integer((riskfun(w[1:5], Data[, 1:5], risk = "cvar",
        alpha = 0.05) + riskfun(w[6:15], Data[, 6:15], risk = "cvar", alpha = 0.05)) >=
        m5)

    #6. CDaR and properties
    m6 <- riskfun(w, Data, risk = "cdar", alpha = 0.05)
    # scaling property: YES*
    pmatrix[1, 6] <- as.integer(round(riskfun(w, abs(b) * Data, risk = "cdar",
        alpha = 0.05)/abs(b), 12) == round(m6, 12))
    # location invariant: NO[1]/NO[2]
    pmatrix[2, 6] <- as.integer(round(riskfun(w, a + Data, risk = "cdar",
        alpha = 0.05), 12) == round(m6, 12))
    pmatrix[3, 6] <- as.integer(round(riskfun(w, a + Data, risk = "cdar",
        alpha = 0.05) + a, 12) == round(m6, 12))
    # Subadditivity: YES
    pmatrix[4, 6] <- as.integer((riskfun(w[1:5], Data[, 1:5], risk = "cdar",
        alpha = 0.05) + riskfun(w[6:15], Data[, 6:15], risk = "cdar", alpha = 0.05)) >=
        m6)

    #7. LPM[m, t=C] and properties
    m7 <- riskfun(w, Data, risk = "lpm", moment = 1, threshold = 0.02)
    # scaling property: YES*
    pmatrix[1, 7] <- as.integer(round(riskfun(w, abs(b) * Data, risk = "lpm",
        moment = 1, threshold = abs(b) * 0.02)/abs(b), 12) == round(m7,
        12))
    # location invariant: YES[1]/NO[2]
    pmatrix[3, 7] <- as.integer(round(riskfun(w, a + Data, risk = "lpm",
        moment = 1, threshold = a + 0.02), 12) == round(m7, 12))
    # Subadditivity: NO
    pmatrix[4, 7] <- as.integer((riskfun(w[1:5], Data[, 1:5], risk = "lpm",
        moment = 1, threshold = 0.02) + riskfun(w[6:15], Data[, 6:15],
        risk = "lpm", moment = 1, threshold = 0.02)) >= m7)
    ...
    # scaling property: YES*
    pmatrix[1, 8] <- as.integer(round(riskfun(w, abs(b) * Data, risk = "lpm",
        moment = 2, threshold = 999)/abs(b), 12) == round(m8, 12))
    # location invariant: YES[1]/NO[2]
    pmatrix[2, 8] <- as.integer(round(riskfun(w, a + Data, risk = "lpm",
        moment = 2, threshold = 999), 12) == round(m8, 12))
    # Subadditivity: YES
    pmatrix[4, 8] <- as.integer((riskfun(w[1:5], Data[, 1:5], risk = "lpm",
        moment = 2, threshold = 999) + riskfun(w[6:15], Data[, 6:15], risk = "lpm",
        moment = 2, threshold = 999)) >= m8)
    ...
    pp <- ecdf(sort(Data %*% w))
    plot(sort(Data %*% w), pp(sort(Data %*% w)), type = "l", col = "black",
        ylab = "P", xlab = "x", main = "CDF")
    ...
    legend("topleft", c("MAD", "SD", "Min", "CVaR[5%]", "LPM[m=2,t=mu]"),
        col = c("green", "orange", "yellow", "tomato1", "steelblue"), pch = rep(21,
            5), pt.bg = c("green", "orange", "yellow", "tomato1", "steelblue"),
        bty = "n")
    dev.off()
...
parma.test2 <- function() {
    ...
    pspec1 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[1], target = mean(colMeans(Data)),
        targetType = "equality", riskType = c("minrisk", "optimal")[1],
    ...
    pspec2 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[2], target = mean(colMeans(Data)),
        targetType = "equality", riskType = c("minrisk", "optimal")[1],
        LB = rep(0, 15), UB = rep(1, 15), budget = 1)
    ...
    pspec3 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[3], target = mean(colMeans(Data)),
        targetType = "equality", riskType = c("minrisk", "optimal")[1],
        options = list(alpha = 0.05), LB = rep(0, 15), UB = rep(1, 15),
        budget = 1)
    ...
    pspec4 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[4], target = mean(colMeans(Data)),
        targetType = "equality", riskType = c("minrisk", "optimal")[1],
        options = list(alpha = 0.05), LB = rep(0, 15), UB = rep(1, 15),
        budget = 1)
    ...
    pspec5 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[5], target = mean(colMeans(Data)),
        targetType = "equality", riskType = c("minrisk", "optimal")[1],
        LB = rep(0, 15), UB = rep(1, 15), budget = 1)

    pspec5q <- parmaspec(S = cov(Data), forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[5], target = mean(colMeans(Data)),
        targetType = "equality", riskType = c("minrisk", "optimal")[1],
        LB = rep(0, 15), UB = rep(1, 15), budget = 1)
    ...
    pspec6 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[6], target = mean(colMeans(Data)),
        targetType = "equality", riskType = c("minrisk", "optimal")[1],
        options = list(threshold = 999, moment = 1), LB = rep(0, 15), UB = rep(1,
            15), budget = 1)
    ...
    rownames(errmat) <- c("MSE[weights]", "MAE[weights]", "MaxE[weights]",
        "Err[risk]")
...
parma.test3 <- function() {
    ...
    pspec1 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[1], target = NULL, targetType = "equality",
        riskType = c("minrisk", "optimal")[2], LB = rep(0, 15), UB = rep(1,
            15), budget = 1)
    ...
    d1 <- rbind(d1, cbind(parmarisk(sol1lpopt)/parmareward(sol1lpopt),
        parmarisk(sol1nlpopt)/parmareward(sol1nlpopt)))
    ...
    pspec2 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[2], target = NULL, targetType = "equality",
        riskType = c("minrisk", "optimal")[2], LB = rep(0, 15), UB = rep(0.5,
            15), budget = 1)
    ...
    pspec3 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[3], target = mean(colMeans(Data)),
        targetType = "equality", riskType = c("minrisk", "optimal")[2],
        options = list(alpha = 0.05), LB = rep(0, 15), UB = rep(1, 15),
        budget = 1)
    ...
    pspec7 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[6], target = mean(colMeans(Data)),
        targetType = "equality", riskType = c("minrisk", "optimal")[2],
        options = list(threshold = 999, moment = 2), LB = rep(0, 15), UB = rep(1,
            15), budget = 1)

    sol7nlpopt <- parmasolve(pspec7, type = "NLP")

    pspec7x <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[6], target = mean(colMeans(Data)),
        targetType = "equality", riskType = c("minrisk", "optimal")[2],
        options = list(threshold = 999, moment = 3), LB = rep(0, 15), UB = rep(1,
            15), budget = 1)
    sol7xnlpopt <- parmasolve(pspec7x, type = "NLP")

    skewness <- function(X) {
        apply(X, 2, FUN = function(x) mean((x - mean(x))^3)/sd(x)^3)
    }
    mcorr <- function(X) {
        apply(cor(X), 2, FUN = function(x) mean(x[x != 1]))
    }
    d7 <- cbind(weights(sol6nlpopt), weights(sol7nlpopt), weights(sol7xnlpopt),
        apply(Data, 2, "sd"), skewness(Data), mcorr(Data))
    d7 <- rbind(d7, cbind(parmarisk(sol6nlpopt), parmarisk(sol7nlpopt),
        parmarisk(sol7xnlpopt), NA, NA, NA))
    d7 <- rbind(d7, cbind(parmareward(sol6nlpopt), parmareward(sol7nlpopt),
        parmareward(sol7xnlpopt), NA, NA, NA))
    d7 <- rbind(d7, cbind(as.numeric(tictoc(sol6nlpopt)), as.numeric(tictoc(sol7nlpopt)),
        as.numeric(tictoc(sol7xnlpopt)), NA, NA, NA))
...
parma.test4 <- function() {
    ...
    spec1 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = "LPM",
        riskType = "optimal", options = list(moment = 1, threshold = 999),
        LB = rep(-0.5, 15), UB = rep(0.5, 15), leverage = 1)

    spec2 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = "LPM",
        riskType = "optimal", options = list(moment = 2, threshold = 999),
        LB = rep(-0.5, 15), UB = rep(0.5, 15), leverage = 1)

    spec3 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = "LPM",
        riskType = "optimal", options = list(moment = 3, threshold = 999),
        LB = rep(-0.5, 15), UB = rep(0.5, 15), leverage = 1)

    spec4 <- parmaspec(scenario = Data, forecast = colMeans(Data), risk = "LPM",
        riskType = "optimal", options = list(moment = 4, threshold = 999),
        LB = rep(-0.5, 15), UB = rep(0.5, 15), leverage = 1)
    ...
    sol1nlp <- parmasolve(spec1, solver.control = list(ftol_rel = 1e-12,
        ftol_abs = 1e-12, xtol_rel = 1e-12, maxeval = 5000, print_level = 1))
    ...
    S1sol <- round(cbind(weights(sol1nlp), weights(sol2nlp), weights(sol3nlp),
        weights(sol4nlp)), 4)
    idx <- unique(which(abs(S1sol) > 0.001, arr.ind = TRUE)[, 1])
    S1sol <- S1sol[idx, ]
    S1sol <- rbind(S1sol, cbind(parmarisk(sol1nlp)/parmareward(sol1nlp),
        parmarisk(sol2nlp)/parmareward(sol2nlp), parmarisk(sol3nlp)/parmareward(sol3nlp),
        parmarisk(sol4nlp)/parmareward(sol4nlp)))
    rownames(S1sol)[dim(S1sol)[1]] <- "Risk/Reward"
    S1sol <- rbind(S1sol, cbind(tictoc(sol1nlp), tictoc(sol2nlp), tictoc(sol3nlp),
        tictoc(sol4nlp)))
...
parma.test5 <- function() {
    ...
    pspec1min <- parmaspec(scenario = Data[, idx], benchmark = benchmark,
        forecast = activeforc, target = target, targetType = "equality",
        risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[1], riskType = c("minrisk",
            "optimal")[1], LB = -wb, UB = rep(0.2, 10), budget = 0)
    ...
    pspec2min <- parmaspec(scenario = Data[, idx], benchmark = benchmark,
        forecast = activeforc, target = target, targetType = "equality",
        risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[2], riskType = c("minrisk",
            "optimal")[1], LB = -wb, UB = rep(0.2, 10), budget = 0)
    ...
    pspec3min <- parmaspec(scenario = Data[, idx], benchmark = benchmark,
        forecast = activeforc, target = target, targetType = "equality",
        risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[3], riskType = c("minrisk",
            "optimal")[1], options = list(alpha = 0.05), LB = -wb, UB = rep(0.2,
            10), budget = 0)
    ...
    pspec4min <- parmaspec(scenario = Data[, idx], benchmark = benchmark,
        forecast = activeforc, target = target, targetType = "equality",
        risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[4], riskType = c("minrisk",
            "optimal")[1], options = list(alpha = 0.05), LB = -wb, UB = rep(0.2,
            10), budget = 0)
    ...
    pspec5min <- parmaspec(scenario = Data[, idx], benchmark = benchmark,
        forecast = activeforc, target = target, targetType = "equality",
        risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[5], riskType = c("minrisk",
            "optimal")[1], LB = -wb, UB = rep(0.2, 10), budget = 0)
    S <- cov(Data[, idx])
    bS <- cov(cbind(benchmark, Data[, idx]))[1, 1:11]
    pspec5qmin <- parmaspec(S = S, benchmarkS = bS, forecast = activeforc,
        target = target, targetType = "equality", risk = c("MAD", "MiniMax",
            "CVaR", "CDaR", "EV", "LPM")[5], riskType = c("minrisk", "optimal")[1],
    ...
    pspec6min <- parmaspec(scenario = Data[, idx] - matrix(benchmark, ncol = 10,
        nrow = nrow(Data)), benchmark = NULL, forecast = activeforc, target = target,
        targetType = "equality", risk = c("MAD", "MiniMax", "CVaR", "CDaR",
            "EV", "LPM")[6], riskType = c("minrisk", "optimal")[1], options = list(moment = 1,
            threshold = 0), LB = -wb, UB = rep(0.2, 10), budget = 0)
    ...
    rownames(M) <- c(colnames(Data[, idx]), "Active_Risk", "Active_Reward",
        "w-MSE(vsNLP)", "w-MAE(vsNLP)", "w-maxErr(vsNLP)", "r-Diff(vsNLP)",
        "t-Diff(vsNLP)")
    ...
    M[16, ] <- M[11, ] - nlpr
    M[17, ] <- as.numeric(c(tictoc(sol1lpmin) - tictoc(sol1nlpmin), tictoc(sol2lpmin) -
        tictoc(sol2nlpmin), tictoc(sol3lpmin) - tictoc(sol3nlpmin), NA,
        tictoc(sol5qpmin) - tictoc(sol5nlpmin), tictoc(sol6lpmin) - tictoc(sol6nlpmin)))
...
parma.test6 <- function() {
    ...
    pspec1opt <- parmaspec(scenario = Data[, idx], benchmark = benchmark,
        forecast = activeforc, risk = c("MAD", "MiniMax", "CVaR", "CDaR",
            "EV", "LPM")[1], riskType = c("minrisk", "optimal")[2], LB = -wb,
        UB = rep(0.2, 10), budget = 0)
    ...
    pspec2opt <- parmaspec(scenario = Data[, idx], benchmark = benchmark,
        forecast = activeforc, risk = c("MAD", "MiniMax", "CVaR", "CDaR",
            "EV", "LPM")[2], riskType = c("minrisk", "optimal")[2], LB = -wb,
        UB = rep(0.2, 10), budget = 0)

    sol2lpopt <- parmasolve(pspec2opt, type = "LP", verbose = TRUE)
    sol2nlpopt <- parmasolve(pspec2opt, type = "NLP", solver.control = list(print_level = 1,
        xtol_rel = 1e-14, ftol_rel = 1e-14))


    pspec3opt <- parmaspec(scenario = Data[, idx], benchmark = benchmark,
        forecast = activeforc, risk = c("MAD", "MiniMax", "CVaR", "CDaR",
            "EV", "LPM")[3], riskType = c("minrisk", "optimal")[2], options = list(alpha = 0.05),
        LB = -wb, UB = rep(0.2, 10), budget = 0)
    ...
    pspec4opt <- parmaspec(scenario = Data[, idx], benchmark = benchmark,
        forecast = activeforc, risk = c("MAD", "MiniMax", "CVaR", "CDaR",
            "EV", "LPM")[4], riskType = c("minrisk", "optimal")[2], options = list(alpha = 0.05),
        LB = -wb, UB = rep(0.2, 10), budget = 0)
    ...
    pspec5opt <- parmaspec(scenario = Data[, idx], benchmark = benchmark,
        forecast = activeforc, risk = c("MAD", "MiniMax", "CVaR", "CDaR",
            "EV", "LPM")[5], riskType = c("minrisk", "optimal")[2], LB = -wb,
        UB = rep(0.2, 10), budget = 0)
    S <- cov(Data[, idx])
    bS <- cov(cbind(benchmark, Data[, idx]))[1, 1:11]
    pspec5qopt <- parmaspec(S = S, benchmarkS = bS, forecast = activeforc,
        risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[5], riskType = c("minrisk",
            "optimal")[2], LB = -wb, UB = rep(0.2, 10), budget = 0)
    ...
    # LPM does not admit a benchmark, instead pass the excesss benchmark returns
    # and set threshold to zero
    pspec6opt <- parmaspec(scenario = Data[, idx] - matrix(benchmark, ncol = 10,
        nrow = nrow(Data)), benchmark = NULL, forecast = activeforc, risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[6], riskType = c("minrisk",
        "optimal")[2], options = list(moment = 1, threshold = 0), LB = -wb,
        UB = rep(0.2, 10), budget = 0)
    ...
    M[15, ] <- apply(abs(M[1:10, ] - nlpw), 2, FUN = function(x) max(x))
    M[16, ] <- M[11, ] - nlpr
    M[17, ] <- as.numeric(c(tictoc(sol1lpopt) - tictoc(sol1nlpopt), tictoc(sol2lpopt) -
        tictoc(sol2nlpopt), tictoc(sol3lpopt) - tictoc(sol3nlpopt), NA,
        tictoc(sol5qpopt) - tictoc(sol5nlpopt), tictoc(sol6lpopt) - tictoc(sol6nlpopt)))
...
parma.test6 <- function() {
    ...
    pspec1 <- parmaspec(scenario = Data[, -6], forecast = colMeans(Data[,
        -6]), risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[3],
        riskType = c("minrisk", "optimal")[2], options = list(alpha = 0.05),
        LB = rep(0, 14), UB = rep(0.2, 14), budget = 1)

    pspec2 <- parmaspec(scenario = Data[, -6], forecast = colMeans(Data[,
        -6]), risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[3],
        riskType = c("minrisk", "optimal")[2], options = list(alpha = 0.05),
        LB = rep(0, 14), UB = rep(0.2, 14), budget = 1, ineq.mat = sectmatrix,
        ineq.LB = c(0, 0, 0), ineq.UB = c(0.4, 0.4, 0.4))
    ...
    secfun <- function(w, optvars, uservars) {
        ineqc <- w[optvars$midx] * c(0, 0, 0) - uservars$sectmatrix %*%
            w[optvars$widx]
    ...
    pspec3 <- parmaspec(scenario = Data[, -6], forecast = colMeans(Data[,
        -6]), risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[3],
        riskType = c("minrisk", "optimal")[2], options = list(alpha = 0.05),
        LB = rep(0, 14), UB = rep(0.2, 14), budget = 1, ineqfun = list(secfun),
        ineqgrad = list(secjac), uservars = uservars)

    sol3nlp <- parmasolve(pspec3, type = "NLP", solver.control = list(print_level = 1))


    S1sol <- round(cbind(weights(sol1lp), weights(sol2lp), weights(sol3nlp)),
        4)
    ...
    S1sol <- rbind(S1sol, cbind(sol1lp@solution$VaR, sol2lp@solution$VaR,
        sol3nlp@solution$VaR))
    rownames(S1sol)[dim(S1sol)[1]] <- "VaR"
    S1sol <- rbind(S1sol, cbind(parmareward(sol1lp), parmareward(sol2lp),
        parmareward(sol3nlp)))
    rownames(S1sol)[dim(S1sol)[1]] <- "Reward"
    S1sol <- rbind(S1sol, cbind(parmarisk(sol1lp)/parmareward(sol1lp),
        parmarisk(sol2lp)/parmareward(sol2lp), parmarisk(sol3nlp)/parmareward(sol3nlp)))
    rownames(S1sol)[dim(S1sol)[1]] <- "Risk/Reward"
...
parma.test7 <- function() {
    ...
    sqfun <- function(w, optvars, uservars) {
        x <- w[optvars$widx]
        return(as.numeric(x[c(1, 2, 8, 9, 10)] %*% uservars$Smatrix %*%
            x[c(1, 2, 8, 9, 10)]) - w[optvars$midx] * (0.01)^2)
    }
    ...
    sqjac <- function(w, optvars, uservars) {
        ...
        widx <- optvars$widx
        x <- w[widx]
        g[1, widx[c(1, 2, 8, 9, 10)]] <- as.numeric(2 * x[c(1, 2, 8, 9,
            10)] %*% uservars$Smatrix)
    ...
    pspec <- parmaspec(scenario = Data[, -6], forecast = colMeans(Data),
        risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[3], riskType = c("minrisk",
            "optimal")[2], options = list(alpha = 0.05), LB = rep(0, 14),
        UB = rep(0.5, 14), budget = 1, ineqfun = list(secfun, sqfun), ineqgrad = list(secjac,
            sqjac), uservars = uservars)
    ...
    secfun <- function(w, optvars, uservars) {
        ...
        ineqc <- c(ineqc, uservars$sectmatrix %*% w[optvars$widx] - c(0.4,
            0.4, 0.4))
        return(ineqc)
    }
    secjac <- function(w, optvars, uservars) {
    ...
    pspec2 <- parmaspec(scenario = Data[, -6], forecast = colMeans(Data),
        target = parmareward(sol1nlp), targetType = "equality", risk = c("MAD",
            "MiniMax", "CVaR", "CDaR", "EV", "LPM")[3], riskType = c("minrisk",
            "optimal")[1], options = list(alpha = 0.05), LB = rep(0, 14),
        UB = rep(0.5, 14), budget = 1, ineqfun = list(secfun, sqfun), ineqgrad = list(secjac,
            sqjac), uservars = uservars)
    ...
    sol1C2 <- w1[c(1, 2, 8, 9, 10)] %*% uservars$Smatrix %*% w1[c(1, 2,
        8, 9, 10)]

    w2 <- weights(sol2nlp)
    sol2C1 <- as.numeric(sectmatrix %*% w2)
    sol2C2 <- w2[c(1, 2, 8, 9, 10)] %*% uservars$Smatrix %*% w2[c(1, 2,
        8, 9, 10)]
    ...
    rownames(S1sol)[dim(S1sol)[1]] <- "Sect_3"
    S1sol <- rbind(S1sol, cbind(sol1C2[1] * 1e+05, sol2C2[1] * 1e+05))
...
parma.test9 <- function() {
    ...
    f <- colMeans(Data)
    pspec1 <- parmaspec(scenario = Data, target = 0, targetType = "equality",
        forecast = f, risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV",
            "LPM")[3], riskType = c("minrisk", "optimal")[1], options = list(alpha = 0.05),
        LB = rep(0, 15), UB = rep(1, 15), budget = 1)
    ...
    pspec2 <- parmaspec(scenario = Data, forecast = f, risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[3], riskType = c("minrisk",
        "optimal")[2], options = list(alpha = 0.05), LB = rep(0, 15), UB = rep(1,
        15), budget = 1)
    optlp <- parmasolve(pspec2, type = "LP", solver = "GLPK")

    postscript("parma_test9-1.eps", width = 12, height = 8)
    plot(r1, rw1, type = "p", col = "steelblue", xlab = "E[Risk]", ylab = "E[Return]",
        main = "CVaR Frontier")
    lines(r2, rw2, col = "tomato1")
    points(r1[opt], rw1[opt], col = "orange", pch = 4, lwd = 3)
    points(parmarisk(optlp), parmareward(optlp), col = "yellow", pch = 6,
        lwd = 3)
    legend("topleft", c("LP Frontier", "NLP Frontier", paste("Frontier Optimal (",
        round(r1[opt]/rw1[opt], 3), ")", sep = ""), paste("Fractional Optimal (",
        round(parmarisk(optlp)/parmareward(optlp), 3), ")", sep = "")),
        col = c("steelblue", "tomato1", "orange", "yellow"), pch = c(1,
            -1, 4, 6), lwd = c(1, 1, 3, 3), lty = c(0, 1, 0, 0), bty = "n",
        cex = 0.9)
    grid()
    mtext(paste("parma package", sep = ""), side = 4, adj = 0, padj = -0.6,
        col = "grey50", cex = 0.7)
    dev.off()
    ...
    # subadditive)\t
    S <- cov(Data)
    pspec2 <- parmaspec(S = S, target = 0, targetType = "equality", forecast = f,
        risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV", "LPM")[5], riskType = c("minrisk",
            "optimal")[1], LB = rep(0, 15), UB = rep(1, 15), budget = 1)
    fqp <- parmafrontier(pspec2, n.points = 200)
    r1 <- sqrt(na.omit(fqp[, "EV"]))
    rw1 <- na.omit(fqp[, "reward"])
    opt <- which(r1/rw1 == min(r1/rw1))

    pspec2 <- parmaspec(scenario = Data, target = 0, targetType = "equality",
        forecast = f, risk = c("MAD", "MiniMax", "CVaR", "CDaR", "EV",
            "LPM")[5], riskType = c("minrisk", "optimal")[1], LB = rep(0,
            15), UB = rep(1, 15), budget = 1)
    ...
    pspec2 <- parmaspec(S = S, forecast = f, risk = c("MAD", "MiniMax",
        "CVaR", "CDaR", "EV", "LPM")[5], riskType = c("minrisk", "optimal")[2],
        LB = rep(0, 15), UB = rep(1, 15), budget = 1)
    optqp <- parmasolve(pspec2)

    pspec2 <- parmaspec(scenario = Data, forecast = f, risk = c("MAD",
        "MiniMax", "CVaR", "CDaR", "EV", "LPM")[5], riskType = c("minrisk",
        "optimal")[2], LB = rep(0, 15), UB = rep(1, 15), budget = 1)
    ...
    lines(r2, rw2, col = "tomato1")
    points(r1[opt], rw1[opt], col = "orange", pch = 4, lwd = 3)
    points(sqrt(parmarisk(optqp)), parmareward(optqp), col = "yellow",
        pch = 6, lwd = 3)
    legend("topleft", c("QP Frontier", "NLP Frontier", paste("Frontier Optimal (",
        round(r1[opt]/rw1[opt], 3), ")", sep = ""), paste("Fractional Optimal (",
        round(sqrt(parmarisk(optqp))/parmareward(optqp), 3), ")", sep = "")),
        col = c("steelblue", "tomato1", "orange", "yellow"), pch = c(1,
            -1, 4, 6), lwd = c(1, 1, 3, 3), lty = c(0, 1, 0, 0), bty = "n",
        cex = 0.9)
    grid()
...
parma.test10 <- function() {
    ...
    spec <- gogarchspec(mean.model = list(model = c("constant", "AR", "VAR")[2],
        lag = 1), distribution.model = c("mvnorm", "manig", "magh")[2],
        ica = "fastica")
    ...
    S1 <- fmoments(Data = Data[, m], n.ahead = 1, roll = 200, spec = spec,
        solver = "hybrid", cluster = cl, gfun = "tanh", maxiter1 = 1e+05,
        rseed = 500)
    ...
    for (i in 1:200) {
        ...
        tmp <- parmautility(U = "CARA", method = "moment", M1 = S1@moments$forecastMu[i,
            ], M2 = S1@moments$forecastCov[, , i], M3 = S1@moments$forecastM3[,
            , i], M4 = S1@moments$forecastM4[, , i], RA = 25, budget = 1,
            LB = rep(0, 12), UB = rep(0.5, 12))
        w[i, ] <- weights(tmp)
    }
    port <- as.numeric(apply(cbind(w, actual), 1, FUN = function(x) sum(x[1:12] *
        x[13:24])))
    ...
    for (i in 1:200) {
        pexsigma[i] <- sqrt(w[i, ] %*% S1@moments$forecastCov[, , i] %*%
            w[i, ])
        ewsigma[i] <- sqrt(ew %*% S1@moments$forecastCov[, , i] %*% ew)

        pexpskew[i] <- (w[i, ] %*% S1@moments$forecastM3[, , i] %*% kronecker(w[i,
            ], w[i, ]))/pexsigma[i]^2
        ewskew[i] <- (ew %*% S1@moments$forecastM3[, , i] %*% kronecker(ew,
            ew))/ewsigma[i]^2

        pexpkurt[i] <- (w[i, ] %*% S1@moments$forecastM4[, , i] %*% kronecker(w[i,
            ], kronecker(w[i, ], w[i, ])))/pexsigma[i]^4
        ewkurt[i] <- (ew %*% S1@moments$forecastM4[, , i] %*% kronecker(ew,
            kronecker(ew, ew)))/ewsigma[i]^4
    }

    postscript("parma_test10.eps", width = 12, height = 8)
    par(mfrow = c(2, 2))
    plot(cumprod(1 + port), type = "l", col = "steelblue", xlab = "Time",
        ylab = "Wealth", main = "CARA Utility Optimization", ylim = c(0.85,
            1.3))
    lines(cumprod(1 + ewport), col = "tomato1")
    legend("topleft", c("CARA (4 Mom)", "EqualWeight"), col = c("steelblue",
        "tomato1"), lty = c(1, 1), bty = "n", cex = 0.9)
    grid()
    mtext(paste("parma package", sep = ""), side = 4, adj = 0, padj = -0.6,
        col = "grey50", cex = 0.7)

    plot(pexsigma, type = "l", col = "steelblue", xlab = "Time", ylab = "Forecast Optimal Sigma",
        main = "Optimal Sigma", ylim = c(min(min(pexsigma), min(ewsigma)),
            max(max(pexsigma), max(ewsigma))))
    lines(ewsigma, col = "tomato1")
    legend("topright", c("CARA (4 Mom)", "EqualWeight"), col = c("steelblue",
        "tomato1"), lty = c(1, 1), bty = "n", cex = 0.9)
    grid()
    mtext(paste("parma package", sep = ""), side = 4, adj = 0, padj = -0.6,
        col = "grey50", cex = 0.7)
    ...
    plot(pexpskew, type = "l", col = "steelblue", xlab = "Time", ylab = "Forecast Optimal Skewness",
        main = "Optimal Skewness", ylim = c(min(min(pexpskew), min(ewskew)),
            max(max(pexpskew), max(ewskew))))
    ...
    plot(pexpkurt, type = "l", col = "steelblue", xlab = "Time", ylab = "Forecast Optimal Kurtosis",
        main = "Optimal Kurtosis", ylim = c(min(min(pexpkurt), min(ewkurt)),
            max(max(pexpkurt), max(ewkurt))))
...
parma.test11 <- function() {
    ...
    spec <- parmaspec(S = S, forecast = fmu, target = fmub, LB = LB, UB = UB,
        risk = "EV", riskType = "minrisk", targetType = "equality", ineq.mat = sectors,
        ineq.LB = 0, ineq.UB = 0.3, budget = 1)
    ...
    cat("\nSOCP - QP")
    cat("\n--------------------------------------------\n")
    print(round(matrix(weights(sol.socp) - weights(sol.qp), ncol = 1),
        5))
    sink(type = "message")
    sink()
    close(zz)
    ...
    control <- list(abs.tol = 1e-08, rel.tol = 1e-08, Nu = 4, max.iter = 1250,
        BigM.K = 4, BigM.iter = 15)
    ...
    plot(frontqp[, "EV"], frontqp[, "reward"], type = "l", xlab = "risk",
        ylab = "reward", main = "EV Frontier\n[QP and SOCP]", cex.main = 0.9)
    points(frontsocp[, "EV"], frontsocp[, "reward"], col = 2)
    legend("topleft", c("QP", "SOCP"), col = 1:2, bty = "n", lty = c(1,
        0), pch = c(-1, 1))
    dev.off()

    # Example 2: Minimize Risk, Subject to return
    # plus inequality constraint and benchmark
    control <- list(abs.tol = 1e-09, rel.tol = 1e-09, Nu = 5, max.iter = 1250,
        BigM.K = 4, BigM.iter = 15)
    LB <- rep(0, 15)
    UB <- rep(0.5, 15)
    set.seed(100)
    ...
    spec <- parmaspec(S = S, benchmarkS = bS, forecast = fmu - mean(B),
        target = fmub - mean(B), LB = LB, UB = UB, risk = "EV", riskType = "minrisk",
        targetType = "inequality", ineq.mat = sectors, ineq.LB = 0, ineq.UB = 0.5,
        budget = 1)
    ...
    # frontier
    spec <- parmaspec(S = S, benchmarkS = bS, forecast = fmu - mean(B),
        target = NULL, LB = LB, UB = UB, risk = "EV", riskType = "minrisk",
        targetType = "equality", ineq.mat = sectors, ineq.LB = 0, ineq.UB = 0.5,
        budget = 1)
    control <- list(abs.tol = 1e-08, rel.tol = 1e-08, Nu = 4, max.iter = 1250,
        BigM.K = 4, BigM.iter = 15)
    frontsocp <- parmafrontier(spec, n.points = 100, miny = NULL, maxy = NULL,
    ...
    plot(frontqp[, "EV"], frontqp[, "reward"], type = "l", xlab = "risk",
        ylab = "reward", main = "EV Benchmark-Relative Frontier\n[QP and SOCP]",
        cex.main = 0.9)
    points(frontsocp[, "EV"], frontsocp[, "reward"], col = 2)
    legend("topleft", c("QP", "SOCP"), col = 1:2, bty = "n", lty = c(1,
        0), pch = c(-1, 1))
    ...
    control <- list(abs.tol = 1e-09, rel.tol = 1e-09, Nu = 4, max.iter = 1250,
    ...
    control <- list(abs.tol = 1e-08, rel.tol = 1e-08, Nu = 5, max.iter = 3250,
    ...
    spec <- parmaspec(S = S, forecast = fmu - mean(B), benchmarkS = bS,
        target = NULL, LB = LB, UB = UB, risk = "EV", riskType = "optimal",
        budget = 1)
    ...
    spec1 <- parmaspec(S = S, forecast = fmu, target = frontsocp[1, "reward"],
        LB = LB, UB = UB, risk = "EV", riskType = "minrisk", budget = 1)
    p1 <- parmasolve(spec1, type = "SOCP")

    spec2 <- parmaspec(S = S, forecast = fmu, riskB = sqrt(tail(frontsocp[,
        "EV"], 1)), LB = LB, UB = UB, risk = "EV", riskType = "maxreward",
        budget = 1)
    ...
    plot(frontsocp[, "EV"], frontsocp[, "reward"], type = "l", xlab = "risk",
        ylab = "reward", main = "EV Frontier\n[SOCP]", cex.main = 0.9)
    points(parmarisk(p1), parmareward(p1), col = 2)
    points(parmarisk(p2), parmareward(p2), col = 4)
    points(parmarisk(p3), parmareward(p3), col = 3)
    points(frontsocp[idx, "EV"], frontsocp[idx, "reward"], col = "steelblue",
        pch = 4)
    legend("topleft", c("Frontier", "MinRisk", "Optimal (Analytical)",
        "Optimal (Frontier)", "MaxReward"), col = c(1, 2, 3, "steelblue",
        4), bty = "n", lty = c(1, 0, 0, 0, 0), pch = c(-1, 1, 1, 4, 1))
    dev.off()
    ...
    control <- list(abs.tol = 1e-12, rel.tol = 1e-12, Nu = 4, max.iter = 1250,
        BigM.K = 4, BigM.iter = 15)
    # minrisk
    spec1 <- parmaspec(S = S, forecast = fmu, target = fmub, LB = LB, UB = UB,
        budget = NULL, risk = "EV", riskType = "minrisk", targetType = "equality",
        leverage = 1.5)

    sol.socp <- parmasolve(spec1, solver.control = control, type = "SOCP")


    spec2 <- parmaspec(scenario = R, forecast = fmu, target = fmub, LB = LB,
        UB = UB, risk = "EV", riskType = "minrisk", targetType = "equality",
        leverage = 1.5)
    ...
    print(round(cbind(c(weights(sol.socp), sqrt(parmarisk(sol.socp)), parmareward(sol.socp)),
        c(weights(sol.nlp), sqrt(parmarisk(sol.nlp)), parmareward(sol.nlp))),
        5))
    sink(type = "message")
    sink()
    close(zz)
    ...
    spec1 <- parmaspec(S = S, forecast = fmu, target = fmub, LB = LB, UB = UB,
        budget = 1, risk = "EV", riskType = "minrisk", targetType = "equality",
        leverage = 1.5001)
    ...
    spec2 <- parmaspec(scenario = R, forecast = fmu, target = fmub, LB = LB,
        UB = UB, risk = "EV", riskType = "minrisk", targetType = "equality",
        leverage = 1.5, eqfun = list(eqfun), eqgrad = list(eqfun.jac))
    ...
    control <- list(abs.tol = 0.001, rel.tol = 0.001, Nu = 4, max.iter = 2000,
    ...
    spec2 <- parmaspec(scenario = R, forecast = fmu, target = fmub, LB = LB,
        UB = UB, risk = "EV", riskType = "optimal", targetType = "equality",
        leverage = 1.5, eqfun = list(eqfun), eqgrad = list(eqfun.jac))
    ...
    spec1 <- parmaspec(S = S, Q = list(Q), qB = 0.01, forecast = fmu, target = fmub,
        LB = LB, UB = UB, budget = 1, risk = "EV", riskType = "minrisk",
    ...
    ineq.jac <- function(w, optvars, uservars) {
        ...
        fm <- optvars$fm
        j <- matrix(0, nrow = 1, ncol = fm)
        j[1, widx] <- ((2 * w[widx] %*% uservars$Q))/(2 * sqrt(w[widx] %*%
            uservars$Q %*% w[widx])[1])
        return(j)
    }
    uservars <- list()
    uservars$Q <- Q
    spec2 <- parmaspec(scenario = R, forecast = fmu, target = fmub, LB = LB,
        UB = UB, risk = "EV", riskType = "minrisk", targetType = "inequality",
        budget = 1, ineqfun = list(ineqfun), ineqgrad = list(ineq.jac),
        uservars = uservars)
    ...
    spec1 <- parmaspec(S = S, Q = list(Q), qB = 0.005, forecast = fmu,
        target = fmub, LB = LB, UB = UB, budget = NULL, leverage = 1.5,
    ...
    spec2 <- parmaspec(scenario = R, forecast = fmu, target = fmub, LB = LB,
        UB = UB, risk = "EV", riskType = "minrisk", targetType = "inequality",
        leverage = 1.5, ineqfun = list(ineqfun), ineqgrad = list(ineq.jac),
        uservars = uservars)
...
parma.test13 <- function() {
    ...
    for (i in 1:4) {
        spec <- parmaspec(scenario = S, forecast = colMeans(S), target = median(colMeans(S)),
    ...
    barplot(t(wnew), legend.text = TRUE, space = 0, col = c("red", "coral",
        "steelblue", "cadetblue", "aliceblue"), args.legend = list(bty = "n"),
        main = "MILP Portfolio (#5/15)")
    mtext(paste("parma package", sep = ""), side = 4, adj = 0, padj = -0.6,
        col = "grey50", cex = 0.7)
    dev.off()
    ...
    for (i in 1:4) {
        spec <- parmaspec(scenario = S, forecast = colMeans(S), target = median(colMeans(S)),
            targetType = "inequality", risk = rmeasure[i], riskType = "minrisk",
            options = list(moment = 1, threshold = 999, alpha = 0.05),
            LB = rep(0, 15), UB = rep(0.5, 15), budget = 1, asset.names = colnames(S))
    ...
    res <- data.frame(MAD_LP = w1[1, ], MAD_GNLP = w2[1, ], MiniMax_LP = w1[2,
        ], MiniMax_GNLP = w2[2, ], CVaR_LP = w1[3, ], CVaR_GNLP = w2[3,
        ], LPM_LP = w1[4, ], LPM_GNLP = w2[4, ])
    rrd <- data.frame(MAD_LP = rr1[1, ], MAD_GNLP = rr2[1, ], MiniMax_LP = rr1[2,
        ], MiniMax_GNLP = rr2[2, ], CVaR_LP = rr1[3, ], CVaR_GNLP = rr2[3,
        ], LPM_LP = rr1[4, ], LPM_GNLP = rr2[4, ])
