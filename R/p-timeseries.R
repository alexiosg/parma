...
.numeric.lag <- function(x, n.lag = 1, pad = 0) {
    if (length(n.lag) == 1) {
        ...
        ans <- NULL
        ans <- apply(as.data.frame(n.lag), 1, FUN = function(i) rbind(ans,
            c(rep(pad, i), head(x, length(x) - i))))
    }
    return(ans)
}
...
.matrix.lag <- function(x, n.lag = 1, pad = 0) {
    if (length(n.lag) == 1) {
        ans <- rbind(matrix(pad, ncol = dim(x)[2], nrow = i), head(x, dim(x)[1] -
            n.lag))
    } else {
        ans <- NULL
        for (i in n.lag) {
            ans <- cbind(ans, rbind(matrix(pad, ncol = dim(x)[2], nrow = i),
                head(x, dim(x)[1] - i)))
        }
    }
    return(ans)
...
.timeSeries.lag <- function(x, n.lag = 1, pad = 0) {
    ...
    if (length(n.lag) == 1) {
        ...
        cxnames <- NULL
        for (i in n.lag) cxnames <- c(cxnames, paste(cnames, ".lag", i,
            sep = ""))
        ans <- NULL
        for (i in n.lag) {
            ans <- cbind(ans, rbind(matrix(pad, ncol = dim(x)[2], nrow = i),
