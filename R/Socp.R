...
Socp <- function(f, A, b, C, d, N, x = NULL, z = NULL, w = NULL, control = list()) {
    ...
    if (length(control)) {
        ...
        if (any(nap <- is.na(pos))) {
            warning(paste("unrecognized control element(s) named `", paste(nms[nap],
                collapse = ", "), "' ignored", sep = ""))
            pos <- pos[!nap]
            control <- control[!nap]
        }
        if (length(control))
            control <- c(control, cpos[-pos]) else control <- cpos
    } else {
        control <- cpos
    }
    ...
    if (is.null(x))
        x <- SocpPhase1(f, A, b, N, control)
    if (is.null(z))
        z <- SocpPhase2(f, A, b, N, x, control)
    socp.getwork <- .C("socp_getwork", as.integer(L), as.integer(N), as.integer(n),
        as.integer(max.iter), as.integer(out.mode), as.integer(mhist),
        as.integer(nhist), as.integer(ndbl), as.integer(nint), PACKAGE = "parma")
    ...
    socp <- .C("socp", as.integer(L), as.integer(N), as.integer(n), as.double(f),
        as.double(as.vector(A)), as.double(b), as.double(x), as.double(z),
        as.double(abs.tol), as.double(rel.tol), as.double(target), as.integer(iter),
        as.double(Nu), as.integer(info), as.integer(out.mode), as.double(hist),
        as.double(dblwork), as.integer(intwork), PACKAGE = "parma")
    ans <- list(x = socp[[7]])
    if (!is.null(d)) {
        idx <- cumsum(N)
        ans$z <- socp[[8]][-idx]
    ...
    ans$info <- socp[[14]]
    ans$message <- switch(as.character(socp[[14]]), `0` = "Error (0)",
        `1` = "Absolute tolerance (1)", `2` = "Relative convergence (2)",
        `3` = "Target value (achieved or unachievable) (3)", `4` = "Maximum iterations (4)")
...
SocpPhase1 <- function(f, A, b, N, control) {
    ...
    if (sum(abs(T - A %*% x0)^2)^(1/2) < 10 * .Machine$double.eps * sqrt(m)) {
        ...
        if (alpha >= 0) {
            ...
            f1 <- c(rep(0, n), 1)
            A1 <- cbind(A, T)
            fit <- Socp(f1, A1, b, NULL, NULL, N, x1, NULL, NULL, control = list(target = 0,
                rel.tol = -1))
            if (fit$x[n + 1] >= 0)
...
##
SocpControl <- function(abs.tol = 1e-18, rel.tol = 1e-16, target = 0, max.iter = 500,
    Nu = 10, out.mode = 0, BigM.K = 2, BigM.iter = 5) {
    list(abs.tol = abs.tol, rel.tol = rel.tol, target = target, max.iter = max.iter,
        Nu = Nu, out.mode = out.mode, BigM.K = BigM.K, BigM.iter = BigM.iter)
}
