...
.qp.min <- function(optvars) {
    ...
    if (index[2] == 1) {
        ...
        if (index[3] == 1) {
            # inequality target
            if (!is.null(eq.mat))
                eq.mat <- cbind(matrix(0, nrow = dim(eq.mat)), eq.mat)
            if (!is.null(ineq.mat))
                ineq.mat <- cbind(matrix(0, nrow = dim(ineq.mat)), ineq.mat)
            .ineqx <- parma.con.qpmin1(eq.mat, eqB = eqB, reward = NULL,
                rewardB = NULL, ineq = rbind(cbind(0, forecast), ineq.mat),
            ...
            if (!is.null(ineq.mat))
                ineq.mat <- cbind(matrix(0, nrow = dim(ineq.mat)), ineq.mat)
            .ineqx <- parma.con.qpmin1(eq.mat, eqB = eqB, reward = c(0,
                forecast), rewardB = target, ineq = ineq.mat, ineqLB = ineq.LB,
                ineqUB = ineq.UB, LB = c(-1, LB), UB = c(-1, UB), budget,
                m + 1)
        }
        dvec <- rep(0, m + 1)
    } else {
        ...
        forecast <- matrix(forecast, ncol = m, nrow = 1)
        if (index[3] == 1) {
            .ineqx <- parma.con.qpmin2(eq.mat, eqB = eqB, reward = NULL,
                rewardB = NULL, ineq = rbind(forecast, ineq.mat), ineqLB = c(target,
                  ineq.LB), ineqUB = c(50000, ineq.UB), LB = LB, UB = UB,
                budget, m)
        } else {
            .ineqx <- parma.con.qpmin2(eq = eq.mat, eqB = eqB, reward = forecast,
                rewardB = target, ineq = ineq.mat, ineqLB = ineq.LB, ineqUB = ineq.UB,
                LB = LB, UB = UB, budget, m)
        }
        dvec <- rep(0, m)
    }
    Amat <- .ineqx$Amat
    ...
    return(list(Dmat = Dmat, dvec = dvec, Amat = Amat, bvec = bvec, meq = meq,
        forecast = forecast, S = S, benchvar = sbench[1], m = m, index = index))
}
