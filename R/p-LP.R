...
lpmin.setup <- function(optvars, cdimn) {
    ...
    if (index[3] == 1) {
        .ineqx <- parma.ineq.lpmin(ineq = rbind(optvars$mu, optvars$ineq.mat),
            ineqLB = c(optvars$mutarget, optvars$ineq.LB), ineqUB = c(Inf,
                optvars$ineqUB), cdim = cdimn)
    } else {
    ...
    if (index[3] == 1) {
        ...
        .eqx <- parma.eq.lpmin(optvars$eq.mat, optvars$eqB, reward = optvars$mu,
            rewardB = optvars$mutarget, cdim = cdimn, optvars$budget, m)
    }
    eqcon <- .eqx$eqcon
    eqB <- .eqx$eqB
    eqn <- .eqx$eqn
...
lpopt.setup <- function(optvars, cdimn) {
    ...
    .ineqx <- parma.ineq.lpopt(ineq = optvars$ineq.mat, ineqLB = optvars$ineq.LB,
        ineqUB = optvars$ineq.UB, LB = optvars$LB, UB = optvars$UB, cdim = cdimn)
    ineqcon <- .ineqx$ineqcon
    ineqB <- .ineqx$ineqB
    ineqn <- .ineqx$ineqn
    ineqm <- .ineqx$ineqm
...
lplpm1.minrisk <- function(optvars) {
    ...
    bounds <- list(lower = list(ind = 1L:(m + n + 1), val = c(LB, rep(0,
        n), 1)), upper = list(ind = 1L:(m + n + 1), val = c(UB, rep(Inf,
        n), 1)))
    ans <- list(objL = objL, Amat = Amat, dir = dir, rhs = rhs, bounds = bounds,
        problem.max = FALSE, reward = optvars$mu, dataidx = c(n, m), widx = 1:m)
    rm(Amat)
    rm(Data)
    gc()
    return(ans)
...
lplpm1.optrisk <- function(optvars) {
    ...
    Amat <- cbind(-Data, -diag(n), matrix(optvars$threshold, ncol = 1,
        nrow = n))
    Amat <- rbind(Amat, cbind(matrix(optvars$mu, ncol = m, nrow = 1), matrix(0,
        ncol = n, nrow = 1), matrix(0, ncol = 1, nrow = 1)))
    ...
    bounds <- list(lower = list(ind = 1L:(m + n + 1), val = c(rep(-10000,
        m), rep(0, n), 0.05)), upper = list(ind = 1L:(m + n + 1), val = c(rep(10000,
        m), rep(Inf, n), 10000 * m)))
    ans <- list(objL = objL, Amat = Amat, dir = dir, rhs = rhs, bounds = bounds,
        problem.max = FALSE, reward = optvars$mu, dataidx = c(n, m), widx = 1:m,
        midx = length(objL))
...
lpmad.minrisk <- function(optvars) {
    ...
    bounds <- list(lower = list(ind = 1L:(m + 2 * n + 1), val = c(LB, 1,
        rep(0, 2 * n))), upper = list(ind = 1L:(m + 2 * n + 1), val = c(UB,
        1, rep(Inf, 2 * n))))
    ans <- list(objL = objL, Amat = Amat, dir = dir, rhs = rhs, bounds = bounds,
        problem.max = FALSE, reward = optvars$mu, dataidx = c(n, m), widx = 1:m)
    rm(Amat)
...
lpmad.optrisk <- function(optvars) {
    ...
    LB <- optvars$LB
    UB <- optvars$UB
    Amat <- cbind(-Data, diag(n), -diag(n), matrix(optvars$benchmark, ncol = 1,
        nrow = n))
    Amat <- rbind(Amat, cbind(matrix(optvars$mu, ncol = m, nrow = 1), matrix(0,
        ncol = n, nrow = 1), matrix(0, ncol = n, nrow = 1), matrix(0, ncol = 1,
        nrow = 1)))
    Amat <- rbind(Amat, ineqcon, eqcon)
    Amat <- as.simple_triplet_matrix(Amat)
    rhs <- rep(0, n)
    ...
    bounds <- list(lower = list(ind = 1L:(m + 2 * n + 1), val = c(rep(-10000,
        m), rep(0, 2 * n), 0.05)), upper = list(ind = 1L:(m + 2 * n + 1),
        val = c(rep(10000, m), rep(Inf, 2 * n), 10000 * m)))
    ans <- list(objL = objL, Amat = Amat, dir = dir, rhs = rhs, bounds = bounds,
        problem.max = FALSE, reward = optvars$mu, dataidx = c(n, m), widx = 1:m,
        midx = length(objL))
    rm(Amat)
...
lpminmax.optrisk <- function(optvars) {
    ...
    Amat <- cbind(-Data, matrix(1, ncol = 1, nrow = n), matrix(optvars$benchmark,
        ncol = 1, nrow = n))
    Amat <- rbind(Amat, cbind(matrix(optvars$mu, ncol = m, nrow = 1), matrix(0,
        nrow = 1, ncol = 1), matrix(0, nrow = 1, ncol = 1)))
    Amat <- rbind(Amat, ineqcon, eqcon)
    Amat <- as.simple_triplet_matrix(Amat)
    rhs <- rep(0, n)
    rhs <- c(rhs, 1, as.numeric(ineqB), as.numeric(eqB))
    ...
    bounds <- list(lower = list(ind = 1L:(m + 2), val = c(rep(-10000, m),
        -Inf, 0.05)), upper = list(ind = 1L:(m + 2), val = c(rep(10000,
        m), Inf, 10000 * m)))
...
lpcvar.minrisk <- function(optvars) {
    ...
    bounds <- list(lower = list(ind = 1L:(m + n + 2), val = c(LB, -1, rep(0,
        n), 1)), upper = list(ind = 1L:(m + n + 2), val = c(UB, 1, rep(Inf,
        n), 1)))
...
lpcvar.optrisk <- function(optvars) {
    ...
    Amat <- cbind(-Data, matrix(-1, nrow = n, ncol = 1), -diag(n), matrix(optvars$benchmark,
        ncol = 1, nrow = n))
    Amat <- rbind(Amat, cbind(matrix(optvars$mu, ncol = m, nrow = 1), matrix(0,
        ncol = 1, nrow = 1), matrix(0, ncol = n, nrow = 1), matrix(0, ncol = 1,
        nrow = 1)))
    Amat <- rbind(Amat, ineqcon, eqcon)
    Amat <- as.simple_triplet_matrix(Amat)
    rhs <- rep(0, n)
    ...
    bounds <- list(lower = list(ind = 1L:(m + n + 2), val = c(rep(-10000,
        m), 0, rep(0, n), 0.05)), upper = list(ind = 1L:(m + n + 2), val = c(rep(10000,
        m), 1e+05, rep(Inf, n), 10000 * m)))

    ans <- list(objL = objL, Amat = Amat, dir = dir, rhs = rhs, bounds = bounds,
        problem.max = FALSE, reward = optvars$mu, dataidx = c(n, m), widx = 1:m,
        vidx = m + 1, midx = length(objL))
    rm(Data)
    rm(Amat)
    gc()
    return(ans)
...
lpcdar.minrisk <- function(optvars) {
    ...
    bounds <- list(lower = list(ind = 1L:(m + 2 * n + 2), val = c(LB, -1,
        rep(0, 2 * n), 1)), upper = list(ind = 1L:(m + 2 * n + 2), val = c(UB,
        1, rep(Inf, 2 * n), 1)))
...
lpcdar.optrisk <- function(optvars) {
    ...
    Amat <- rbind(Amat, cbind(matrix(optvars$mu, nrow = 1, ncol = m), matrix(0,
        nrow = 1, ncol = 1), matrix(0, nrow = 1, ncol = n), matrix(0, nrow = 1,
        ncol = n), matrix(0, nrow = 1, ncol = 1)))
    ...
    bounds <- list(lower = list(ind = 1L:(m + 2 * n + 2), val = c(rep(-10000,
        m), -1e+05, rep(0, 2 * n), 1e-12)), upper = list(ind = 1L:(m +
        2 * n + 2), val = c(rep(10000, m), 1e+05, rep(Inf, 2 * n), 10000 *
        m)))
...
lpport <- function(optvars, solver, ...) {
    ...
    if (optvars$index[5] == 1) {
        setup <- switch(risk, mad = lpmad.minrisk(optvars), minimax = lpminmax.minrisk(optvars),
            cvar = lpcvar.minrisk(optvars), cdar = lpcdar.minrisk(optvars),
            lpm = lplpm1.minrisk(optvars))
        sol <- lpminsolver(setup, solver, optvars, ...)
    } else {
        setup <- switch(risk, mad = lpmad.optrisk(optvars), minimax = lpminmax.optrisk(optvars),
            cvar = lpcvar.optrisk(optvars), cdar = lpcdar.optrisk(optvars),
...
lpminsolver <- function(setup, solver, optvars, ...) {
    ...
    m <- setup$dataidx[2]
    sol <- switch(solver, glpk = try(Rglpk_solve_LP(obj = setup$objL, mat = setup$Amat,
        dir = setup$dir, rhs = setup$rhs, bounds = setup$bounds, max = setup$problem.max,
    ...
    if (inherits(sol, "try-error")) {
        ...
        if (optvars$index[4] == 6)
            risk <- risk - 1
        if (optvars$index[4] == 3)
            VaR <- sol$solution[setup$vidx] else VaR <- NA
        if (optvars$index[4] == 4)
            DaR <- sol$solution[setup$vidx] else DaR <- NA
    }
    rm(setup)
    gc()
