...
grad.abs.smooth <- function(x) {
    return(0.5 * (2 * x + 2 * parma.Small)/sqrt((x + parma.Small) * (x +
        parma.Small)))
}
...
grad.abs.smooth2 <- function(x) {
    return((2 * 1/pi) * atan(parma.Big * x) + (2 * x/pi) * 1/(1 + parma.Big *
        x * x))
}
##################################################################
# Variance (EV)
...
grad.optvar <- function(w, optvars, uservars) {
    ...
    fg <- rep(0, fm)
    fg[widx] <- g
    if (optvars$index[2] > 0)
        fg[midx] <- -sum(2 * optvars$benchmark * xport)/N
    return(fg)
}
##################################################################
# MAD
...
grad.minmad <- function(w, optvars, uservars) {
    ...
    for (i in 1:wm) g[i] <- sum(Data[, i] * (parma.Small + xport)/(N *
        sqrt((parma.Small + xport)^2)))
    fg <- rep(0, fm)
    fg[widx] <- g
    return(fg)
}
...
func.optmad <- function(w, optvars, uservars) {
    widx <- optvars$widx
    midx <- optvars$midx
    f <- mean(func.abs.smooth(optvars$Data %*% as.numeric(w[widx]) - w[midx] *
        optvars$benchmark))
    return(f)
}
...
grad.optmad <- function(w, optvars, uservars) {
    ...
    fg[widx] <- g
    if (optvars$index[2] > 0)
        fg[midx] <- -sum(optvars$benchmark * (parma.Small + xport)/(N *
            sqrt((parma.Small + xport)^2)))
    return(fg)
}
##################################################################
...
grad.mincvar <- function(w, optvars, uservars) {
    ...
    port <- Data %*% w[widx]
    for (i in 1:wm) {
        g[i] <- (sum(Data[, i]/(2 * N)) - sum((Data[, i] * (-optvars$benchmark +
            port - w[vidx]))/((2 * N) * (parma.Small + (-optvars$benchmark +
            port - w[vidx])^2)^0.5)))/alpha
    }
    fg <- rep(0, fm)
    fg[widx] <- g
    # derivative wrt to VaR
    fg[vidx] <- (sum((-optvars$benchmark + port - w[vidx])/(2 * N * sqrt(parma.Small +
        (-optvars$benchmark + port - w[vidx])^2))) - 0.5)/alpha + 1
...
grad.optcvar <- function(w, optvars, uservars) {
    ...
    port <- Data %*% w[widx]
    for (i in 1:wm) {
        g[i] <- (sum(Data[, i]/(2 * N)) - sum((Data[, i] * (port - w[vidx] -
            w[midx] * optvars$benchmark))/((2 * N) * (parma.Small + (port -
            w[vidx] - w[midx] * optvars$benchmark)^2)^0.5)))/alpha
    }
    fg <- rep(0, fm)
    fg[widx] <- g
    # the derivative wrt to VaR
    fg[vidx] <- (sum((port - w[vidx] - w[midx] * optvars$benchmark)/(2 *
        N * sqrt(parma.Small + (port - w[vidx] - w[midx] * optvars$benchmark)^2))) -
        0.5)/alpha + 1
    if (optvars$index[2] > 0)
        fg[midx] <- sum(-optvars$benchmark/(2 * N) + optvars$benchmark *
            (port - w[vidx] - w[midx] * optvars$benchmark)/((2 * N) * (parma.Small +
            (port - w[vidx] - w[midx] * optvars$benchmark)^2)^0.5))/(alpha)
    # we reverse the sign since we are minimizing
    return(-1 * fg)
}
##################################################################
...
grad.minlpm <- function(w, optvars, uservars) {
    ...
    pd2 <- pd * pd
    for (i in 1:wm) {
        g[i] <- -((sum((abs(threshold + (parma.Small + pd2)^(1/2) - port))^(moment))/(N *
            2^moment))^(1/moment - 1) * sum(moment * (Data[, i] - (Data[,
            i] * pd)/(parma.Small + pd2)^(1/2)) * (abs(threshold + (parma.Small +
            pd2)^(1/2) - port))^(moment - 1)))/(N * 2^moment * moment)
    }
    fg <- rep(0, fm)
    fg[widx] <- g
    return(fg)
...
grad.optlpm <- function(w, optvars, uservars) {
    ...
    pd2 <- pd * pd
    for (i in 1:wm) {
        g[i] <- -((sum((abs(dp + sqrt(parma.Small + pd2)))^moment)/(N *
            2^moment))^(1/moment - 1) * sum(moment * (Data[, i] - (Data[,
            i] * pd)/sqrt(parma.Small + pd2)) * (abs(dp + sqrt(parma.Small +
            pd2)))^(moment - 1)))/(moment * N * 2^moment)
    }
    fg <- rep(0, fm)
    fg[widx] <- g
    # This is required because of the scaling of the threshold by the fractional parameter
    fg[midx] <- ((sum((dp + sqrt(parma.Small + (port - tm)^2))^moment)/(N *
        2^moment))^(1/moment - 1) * sum(moment * (threshold - (threshold *
        pd)/sqrt(parma.Small + (port - tm)^2)) * (dp + sqrt(parma.Small +
        pd2))^(moment - 1)))/(N * moment * 2^moment)
    return(fg)
}
##################################################################
# MiniMax
...
ineqfun.minminmax <- function(w, optvars, uservars) {
    as.numeric(-w[optvars$vidx] - (as.numeric(optvars$Data %*% w[optvars$widx]) -
        optvars$benchmark))
}
...
ineqfun.optminmax <- function(w, optvars, uservars) {
    as.numeric(-w[optvars$vidx] - optvars$Data %*% w[optvars$widx] + w[optvars$midx] *
        optvars$benchmark)
}

ineqjac.optminmax <- function(w, optvars, uservars) {
    g <- cbind(matrix(-1, nrow = optvars$N), -optvars$Data, matrix(-optvars$benchmark,
        nrow = optvars$N))
    return(g)
}
##################################################################
...
grad.optlpmupm <- function(w, optvars, uservars) {
    ...
    pd2 <- pd * pd
    for (i in 1:wm) {
        g[i] <- -((sum((dp + sqrt(parma.Small + pd2))^lmoment)/(N * 2^lmoment))^(1/lmoment -
            1) * sum(lmoment * (Data[, i] - (Data[, i] * pd)/sqrt(parma.Small +
            pd2)) * (dp + sqrt(parma.Small + pd2))^(lmoment - 1)))/(lmoment *
            N * 2^lmoment)
    }
    fg <- rep(0, fm)
    fg[widx] <- g
    fg[midx] <- ((sum((dp + sqrt(parma.Small + (port - tm)^2))^lmoment)/(N *
        2^lmoment))^(1/lmoment - 1) * sum(lmoment * (lthreshold - (lthreshold *
        pd)/sqrt(parma.Small + (port - tm)^2)) * (dp + sqrt(parma.Small +
        pd2))^(lmoment - 1)))/(N * lmoment * 2^lmoment)
    return(fg)
}
...
grad.ineq.lpmupm <- function(w, optvars, uservars) {
    ...
    pd2 <- pd * pd
    for (i in 1:wm) {
        ans1[i] <- -((sum((pd + sqrt(parma.Small + pd2))^umoment)/(N *
            2^umoment))^(1/umoment - 1) * sum(umoment * (Data[, i] + (Data[,
            i] * pd)/sqrt(parma.Small + pd2)) * (pd + sqrt(parma.Small +
            pd2))^(umoment - 1)))/(umoment * N * 2^umoment)
    }
    fg <- rep(0, fm)
    fg[widx] <- g

    fg[midx] <- (((sum((pd + sqrt(parma.Small + pd2))^umoment)/(N * 2^umoment))^(1/umoment -
        1)) * (sum(umoment * (uthreshold + (uthreshold * pd)/sqrt(parma.Small +
        pd2)) * (pd + sqrt(parma.Small + pd2))^(umoment - 1))))/(umoment *
        N * 2^umoment)
...
ineqjac.lpm.min <- function(w, optvars, uservars) {
    ...
    pd2 <- pd * pd
    for (i in 1:wm) {
        g[i] <- -((sum((threshold + (parma.Small + pd2)^(1/2) - port)^(moment))/(N *
            2^moment))^(1/moment - 1) * sum(moment * (Data[, i] - (Data[,
            i] * pd)/(parma.Small + pd2)^(1/2)) * (threshold + (parma.Small +
            pd2)^(1/2) - port)^(moment - 1)))/(N * 2^moment * moment)
    }
    fg <- rep(0, fm)
    fg[widx] <- g
...
ineqjac.lpm.opt <- function(w, optvars, uservars) {
    ...
    pd2 <- pd * pd
    for (i in 1:wm) {
        g[i] <- -((sum((dp + sqrt(parma.Small + pd2))^moment)/(N * 2^moment))^(1/moment -
            1) * sum(moment * (Data[, i] - (Data[, i] * pd)/sqrt(parma.Small +
            pd2)) * (dp + sqrt(parma.Small + pd2))^(moment - 1)))/(moment *
            N * 2^moment)
    }
    fg <- rep(0, fm)
    fg[widx] <- g
    # This is required because of the scaling of the threshold by the fractional parameter
    fg[midx] <- -uservars$lpmtarget + ((sum((dp + sqrt(parma.Small + (port -
        tm)^2))^moment)/(N * 2^moment))^(1/moment - 1) * sum(moment * (threshold -
        (threshold * pd)/sqrt(parma.Small + (port - tm)^2)) * (dp + sqrt(parma.Small +
        pd2))^(moment - 1)))/(N * moment * 2^moment)
...
nlpport <- function(optvars, uservars, control = list(), ...) {
    ...
    if (optvars$index[5] == 1) {
        if (optvars$index[3] == 2 && is.null(optvars$ineqfun) && xrisk !=
            "minimax") {
        ...
        eval_f <- switch(xrisk, cvar = func.mincvar, mad = func.minmad,
            lpm = func.minlpm, minimax = func.minminmax, ev = func.minvar)

        eval_grad_f <- switch(xrisk, cvar = grad.mincvar, mad = grad.minmad,
            lpm = grad.minlpm, minimax = grad.minminmax, ev = grad.minvar)

        ans <- try(nloptr(x0 = optvars$x0, eval_f = eval_f, eval_grad_f = eval_grad_f,
            lb = optvars$LB, ub = optvars$UB, eval_g_ineq = xparma.ineq.minfun,
            eval_jac_g_ineq = xparma.ineq.mingrad, eval_g_eq = parma.eq.minfun,
            eval_jac_g_eq = parma.eq.mingrad, opts = ctrl, optvars = optvars,
            uservars = uservars), silent = TRUE)
        ...
        if (inherits(ans, "try-error")) {
            ...
            sol$risk <- as.numeric(fun.risk(sol$weights, optvars$Data,
                options = list(alpha = optvars$alpha, moment = optvars$moment,
                  threshold = optvars$threshold), risk = xrisk, benchmark = optvars$benchmark))
            sol$reward <- sum(sol$weights * optvars$mu)
            if (optvars$index[4] == 3)
                sol$VaR <- ans$solution[optvars$vidx]
            if (optvars$index[4] == 4)
                sol$DaR <- ans$solution[optvars$vidx]
        }
    } else {
        eval_f <- switch(xrisk, cvar = func.optcvar, mad = func.optmad,
            lpm = func.optlpm, minimax = func.optminmax, ev = func.optvar)

        eval_grad_f <- switch(xrisk, cvar = grad.optcvar, mad = grad.optmad,
            lpm = grad.optlpm, minimax = grad.optminmax, ev = grad.optvar)

        ans <- try(nloptr(x0 = optvars$x0, eval_f = eval_f, eval_grad_f = eval_grad_f,
            lb = optvars$fLB, ub = optvars$fUB, eval_g_ineq = parma.ineq.optfun,
            eval_jac_g_ineq = parma.ineq.optgrad, eval_g_eq = parma.eq.optfun,
            eval_jac_g_eq = parma.eq.optgrad, opts = ctrl, optvars = optvars,
            uservars = uservars), silent = TRUE)
        ...
        if (inherits(ans, "try-error")) {
            ...
            sol$risk <- as.numeric(fun.risk(sol$weights, optvars$Data,
                options = list(alpha = optvars$alpha, moment = optvars$moment,
                  threshold = optvars$threshold), risk = xrisk, benchmark = optvars$benchmark))
            if (optvars$index[4] == 3)
                sol$VaR <- ans$solution[optvars$vidx]/ans$solution[optvars$midx]
            if (optvars$index[4] == 4)
                sol$DaR <- ans$solution[optvars$vidx]/ans$solution[optvars$midx]
        }
    }
    return(sol)
}
...
.nloptr.ctrl <- function(control = list()) {
    ...
    if (is.null(control$minf_max))
        ctrl$minf_max <- 1e-24 else ctrl$minf_max <- control$minf_max
    if (is.null(control$ftol_rel))
        ctrl$ftol_rel <- 1e-16 else ctrl$ftol_rel <- control$ftol_rel
    if (is.null(control$ftol_abs))
        ctrl$ftol_abs <- 1e-16 else ctrl$ftol_abs <- control$ftol_abs
    if (is.null(control$xtol_rel))
        ctrl$xtol_rel <- 1e-24 else ctrl$xtol_rel <- control$xtol_rel
    if (is.null(control$maxeval))
        ctrl$maxeval <- 10000 else ctrl$maxeval <- as.integer(control$maxeval)
    if (is.null(control$maxtime))
        ctrl$maxtime <- 10000 else ctrl$maxtime <- control$maxtime
    if (is.null(control$print_level))
        ctrl$print_level <- 0 else ctrl$print_level <- as.integer(control$print_level)
    return(ctrl)
}
...
ineqjac.bsturnover.min <- function(w, optvars, uservars) {
    ...
    fm <- optvars$fm
    j <- matrix(0, 2, fm)
    j[1, widx] <- 0.25 * (-2 * uservars$wold + 2 * w[widx])/sqrt((-uservars$wold +
        w[widx])^2 + parma.Small) + 0.5
    j[2, widx] <- 0.25 * (-2 * uservars$wold + 2 * w[widx])/sqrt((uservars$wold -
        w[widx])^2 + parma.Small) - 0.5
    return(j)
}
...
ineqjac.bsturnover.opt <- function(w, optvars, uservars) {
    ...
    j[1, midx] <- sum(-0.5 * tmp * w[widx]/(w[midx]^2 * sqrt(tmp^2 + 1e-20)) -
        0.5 * w[widx]/w[midx]^2)
    tmp <- -w[widx]/w[midx] + uservars$wold
    j[2, widx] <- -0.5 * (tmp/(w[midx] * sqrt(tmp^2 + 1e-20))) - 0.5/w[midx]
    j[2, midx] <- sum(0.5 * tmp * w[widx]/(w[midx]^2 * sqrt(tmp^2 + 1e-20)) +
        0.5 * w[widx]/w[midx]^2)
    return(j)
}
...
ineqjac.variance.opt <- function(w, optvars, uservars) {
    ...
    g[1, midx] <- as.numeric(((tmp2/w[midx]) %*% -uservars$Cov) %*% w[widx] -
        ((tmp1/w[midx]^2) %*% uservars$Cov) %*% w[widx])
    return(g)
}
