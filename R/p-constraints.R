...
parma.ineq.lpopt <- function(ineq, ineqLB, ineqUB, LB, UB, cdim) {
    ...
    if (is.matrix(ineq)) {
        ineqn <- dim(ineq)[1]
        ineqcon <- cbind(rbind(ineq, -ineq), matrix(0, nrow = 2 * ineqn,
            ncol = cdim))
        ineqcon <- rbind(ineqcon, cbind(diag(m), matrix(0, nrow = m, ncol = cdim)),
        ...
        ineqcon[(2 * ineqn + m + 1):(2 * ineqn + 2 * m), m + cdim] <- 1 *
            LB
        ineqB <- c(rep(0, 2 * ineqn), rep(0, 2 * m))
        nn <- 2 * ineqn + 2 * m
    } else {
        ineqn <- 0
        ineqcon <- rbind(cbind(diag(m), matrix(0, nrow = m, ncol = cdim)),
            cbind(-1 * diag(m), matrix(0, nrow = m, ncol = cdim)))
...
parma.eq.lpopt <- function(eq, eqB, reward, rewardB, cdim, budget = 1,
    m = NULL) {
    eqcon <- NULL
    eqn <- 0
    if (!is.null(m))
        eqm <- m else eqm <- length(reward)
    eqB <- NULL
    if (is.matrix(eq)) {
        eqn <- dim(eq)[1]
        eqm <- dim(eq)[2]
        if (!is.null(budget)) {
            ...
            eqcon <- rbind(eqcon, cbind(matrix(1, nrow = 1, ncol = eqm),
                matrix(0, nrow = 1, ncol = cdim)), cbind(eq, matrix(0,
                nrow = eqn, ncol = cdim)))
            ...
            eqcon <- rbind(eqcon, cbind(eq, matrix(0, nrow = eqn, ncol = cdim)))
            eqcon[, eqm + cdim] <- -eqB
            eqn <- eqn
            eqB <- rep(0, eqn)
        }
...
parma.eq.lpmin <- function(eq, eqB, reward, rewardB, cdim, budget = 1,
    m = NULL) {
    mbench <- 0
    if (!is.null(rewardB)) {
        eqcon <- cbind(matrix(reward, nrow = 1), matrix(mbench, nrow = 1,
            ncol = 1), matrix(0, nrow = 1, ncol = cdim - 1))
        eqn <- 1
        if (!is.null(m))
            eqm <- m else eqm <- length(reward)
        eqB <- rewardB
        if (is.matrix(eq)) {
            ...
            if (budget) {
                ...
                eqcon <- rbind(cbind(matrix(1, nrow = 1, ncol = eqm), matrix(0,
                  nrow = 1, ncol = cdim)), cbind(eq, matrix(0, nrow = eqn,
                  ncol = cdim)), eqcon)
                eqn <- eqn + 2
                eqm <- eqm
                eqB <- c(1, eqB, rewardB)
            } else {
                eqcon <- rbind(cbind(eq, matrix(0, nrow = eqn, ncol = cdim)),
                  eqcon)
                eqn <- eqn + 1
                eqB <- c(eqB, rewardB)
            }
...
# QP constraints
################################################################
parma.con.qpmin1 <- function(eq, eqB, reward, rewardB, ineq, ineqLB, ineqUB,
    LB, UB, budget, m) {
    Amat <- NULL
    bvec <- NULL
    meq <- 0
    ...
    if (!is.null(ineq)) {
        nineq <- length(ineqLB)
        Amat <- rbind(Amat, ineq, -ineq)
        bvec <- c(bvec, ineqLB, -ineqUB)
    }
    # Upper and Lower Bounds
    Amat <- rbind(Amat, diag(m), -diag(m))
    bvec <- c(bvec, if (!is.null(LB)) LB else rep(-1000, m), if (!is.null(UB)) -UB else rep(-1000,
        m))

    return(list(Amat = Amat, bvec = bvec, meq = meq))
}

parma.con.qpmin2 <- function(eq, eqB, reward, rewardB, ineq, ineqLB, ineqUB,
    LB, UB, budget, m) {
    Amat <- NULL
    bvec <- NULL
    meq <- 0
...
parma.con.qpopt <- function(eq, eqB, reward, ineq, ineqLB, ineqUB, LB,
    UB, budget, m) {
    Amat <- NULL
    bvec <- NULL
    meq <- 0
    ...
    if (!is.null(ineq)) {
        ...
        Amat <- rbind(Amat, cbind(ineqLB, ineq), cbind(-ineqUB, -ineq))
...
parma.ineq.minfun <- function(w, optvars, uservars) {
    ...
    if (!is.null(optvars$ineqfun)) {
        ...
        if (optvars$index[3] == 1)
            cons <- ineqfun.target.min(w, optvars, uservars) else cons <- NULL
    ...
    if (optvars$index[4] == 2)
        cons <- c(cons, ineqfun.minminmax(w, optvars, uservars))
    return(cons)
}
...
parma.ineq.mingrad <- function(w, optvars, uservars) {
    ...
    if (!is.null(optvars$ineqgrad)) {
        ...
        if (optvars$index[3] == 1)
            cons <- ineqjac.target.min(w, optvars, uservars) else cons <- NULL
        fnlist <- list(w, optvars, uservars)
        names(fnlist) <- c("w", "optvars", "uservars")
        for (i in 1:n) {
    ...
    if (optvars$index[4] == 2) {
        if (is.null(cons))
            cons <- ineqjac.minminmax(w, optvars, uservars) else cons <- rbind(cons, ineqjac.minminmax(w, optvars, uservars))
    }
    return(cons)
}
...
parma.eq.minfun <- function(w, optvars, uservars) {
    if (!is.null(optvars$eqfun)) {
        ...
        if (optvars$index[6] == 0)
            cons <- eqfun.budget.min(w, optvars, uservars) else cons <- eqfun.leverage.min(w, optvars, uservars)
        if (optvars$index[3] == 2)
            cons <- c(cons, eqfun.target.min(w, optvars, uservars))
        fnlist <- list(w, optvars, uservars)
        names(fnlist) <- c("w", "optvars", "uservars")
        for (i in 1:n) {
...
parma.eq.mingrad <- function(w, optvars, uservars) {
    if (!is.null(optvars$eqgrad)) {
        ...
        if (optvars$index[6] == 0)
            cons <- eqjac.budget.min(w, optvars, uservars) else cons <- eqjac.leverage.min(w, optvars, uservars)
        if (optvars$index[3] == 2)
            cons <- rbind(cons, eqjac.target.min(w, optvars, uservars))
        fnlist <- list(w, optvars, uservars)
        names(fnlist) <- c("w", "optvars", "uservars")
        for (i in 1:n) {
...
parma.ineq.optfun <- function(w, optvars, uservars) {
    ...
    if (optvars$index[4] == 2)
        cons <- c(cons, ineqfun.optminmax(w, optvars, uservars))
...
parma.ineq.optgrad <- function(w, optvars, uservars) {
    ...
    if (optvars$index[4] == 2)
        cons <- rbind(cons, ineqjac.optminmax(w, optvars, uservars))
    return(cons)
}
...
parma.eq.optfun <- function(w, optvars, uservars) {
    ...
    if (!is.null(optvars$eqfun)) {
        n <- length(optvars$eqfun)
        if (optvars$index[6] == 0)
            cons <- c(cons, eqfun.budget.opt(w, optvars, uservars)) else cons <- c(cons, eqfun.leverage.opt(w, optvars, uservars))
        fnlist <- list(w, optvars, uservars)
        names(fnlist) <- c("w", "optvars", "uservars")
        for (i in 1:n) {
            cons <- c(cons, do.call(optvars$eqfun[[i]], args = fnlist))
...
parma.eq.optgrad <- function(w, optvars, uservars) {
    ...
    if (!is.null(optvars$eqgrad)) {
        n <- length(optvars$eqgrad)
        if (optvars$index[6] == 0)
            cons <- rbind(cons, eqjac.budget.opt(w, optvars, uservars)) else cons <- rbind(cons, eqjac.leverage.opt(w, optvars, uservars))
        fnlist <- list(w, optvars, uservars)
        names(fnlist) <- c("w", "optvars", "uservars")
        for (i in 1:n) {
            cons <- rbind(cons, do.call(optvars$eqgrad[[i]], args = fnlist))
