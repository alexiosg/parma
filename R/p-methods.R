...
setMethod("show", signature(object = "parmaSpec"), function(object) {
    cat(paste("\n+---------------------------------+", sep = ""))
    cat(paste("\n|       PARMA Specification       |", sep = ""))
    cat(paste("\n+---------------------------------+ ", sep = ""))
    cat(paste("\nNo.Assets\t\t: ", object@model$indx[8], sep = ""))
    tmp <- c("LP", "MILP", "QP", "MIQP", "SOCP", "NLP", "MINLP", "GNLP")[which(object@model$type ==
        1)]
    cat("\nProblem\t\t\t:", paste(tmp, sep = " ", collapse = ","))
    cat(paste("\nInput\t\t\t: ", if (object@model$indx[1] == 1)
        "Scenario" else "Covariance", sep = ""))
...
setMethod("show", signature(object = "parmaPort"), function(object) {
    if (!is.null(object@model$utility)) {
        ...
        cat(paste("\nObjective\t\t: ", round(object@solution$utility, 4),
            sep = ""))
        cat(paste("\nReward\t\t\t: ", round(object@solution$reward, 7),
            sep = ""))
        cat("\n\n")
        w <- weights(object)
        cnames <- object@model$asset.names
        if (is.null(cnames))
            cnames <- paste("A_", 1:length(w), sep = "")
        names(w) <- cnames
...
setGeneric("parmaset<-", function(object, value) {
    standardGeneric("parmaset<-")
})
...
.parmaset <- function(object, value) {
    ...
    valid.choices <- c("scenario", "probability", "S", "benchmark", "benchmarkS",
        ...
        "eqgrad", "uservars", "ineq.mat", "ineq.LB", "ineq.UB", "eq.mat",
        "eqB", "max.pos", "asset.names")
    inc <- NULL
    for (i in seq_along(parnames)) {
        if (is.na(match(parnames[i], valid.choices))) {
            warning((paste("Unrecognized Value: ", parnames[i], "...Ignored",
                sep = "")))
        } else {
            inc <- c(inc, i)
        }
    ...
    # which is NOT NULL)
    if (indx[2] == 1)
        benchmark <- object@modeldata$benchmark else benchmark <- NULL
    if (indx[2] == 1)
        benchmarkS <- object@modeldata$benchmarkS else benchmarkS <- NULL
    ...
    if (length(inc) > 0) {
        for (i in 1:length(inc)) {
            eval(parse(text = paste(parnames[inc[i]], "=unname(value[[inc[i]]])",
                sep = "")))
        }
    }
    newspec <- parmaspec(scenario = scenario, probability = probability,
        S = S, benchmark = benchmark, benchmarkS = benchmarkS, forecast = forecast,
        target = target, targetType = targetType, risk = risk, riskType = riskType,
        options = options, LB = LB, UB = UB, budget = budget, leverage = leverage,
        ineqfun = ineqfun, ineqgrad = ineqgrad, eqfun = eqfun, eqgrad = eqgrad,
        uservars = uservars, ineq.mat = ineq.mat, ineq.LB = ineq.LB, ineq.UB = ineq.UB,
        eq.mat = eq.mat, eqB = eqB, maxp.pos = max.pos, asset.names = asset.names)
    return(newspec)
}

setReplaceMethod(f = "parmaset", signature = c(object = "parmaSpec", value = "vector"),
    definition = .parmaset)
