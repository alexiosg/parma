...
fun.mad <- function(w, Data, benchmark = NULL) {
    ...
    if (is.null(benchmark))
        d <- scale(d, scale = FALSE) else d <- d - benchmark
    return(mean(abs(d)))
}
...
fun.cdar <- function(w, Data, DaR = NULL, alpha = 0.05, lower = TRUE, benchmark = NULL) {
    ...
    if (is.null(DaR))
        DaR <- as.numeric(xdd[intalpha])

    intalpha2 <- max(max(which(cp < alpha)), 1)
    intalpha3 <- max(min(which(cp >= alpha)), 1)

    CDaRplus <- sum(probability[sidx[1:intalpha2]]/sum(probability[sidx[1:intalpha2]]) *
        xdd[1:intalpha2])
...
riskfun <- function(weights, Data, risk = c("mad", "ev", "minimax", "cvar",
    "cdar", "lpm"), benchmark = NULL, alpha = 0.05, moment = 1, threshold = 0,
    VaR = NULL, DaR = NULL) {
    xrisk <- match.arg(tolower(risk[1]), c("mad", "ev", "minimax", "cvar",
        "cdar", "lpm"))
    ans <- switch(tolower(xrisk), mad = fun.mad(weights, Data, benchmark),
        ev = fun.var(weights, Data, benchmark), minimax = fun.minmax(weights,
            Data, benchmark), cvar = fun.cvar(weights, Data, VaR = VaR,
            alpha = alpha, benchmark), lpm = fun.lpm(weights, Data, moment = moment,
            threshold = threshold), cdar = fun.cdar(weights, Data, DaR = DaR,
            alpha = alpha, lower = TRUE, benchmark))
    return(abs(ans))
}

fun.risk <- function(weights, Data, options, risk = c("mad", "ev", "minimax",
    "cvar", "cdar", "lpm"), benchmark = NULL, VaR = NULL, DaR = NULL) {
    xrisk <- match.arg(tolower(risk[1]), c("mad", "ev", "minimax", "cvar",
        "cdar", "lpm"))
    ans <- switch(xrisk, mad = fun.mad(weights, Data, benchmark), ev = fun.var(weights,
        Data, benchmark), minimax = fun.minmax(weights, Data, benchmark),
        cvar = fun.cvar(weights, Data, VaR = VaR, alpha = options$alpha,
            benchmark), lpm = fun.lpm(weights, Data, moment = options$moment,
            threshold = options$threshold), cdar = fun.cdar(weights, Data,
            DaR = DaR, alpha = options$alpha, lower = TRUE, benchmark))
    return(abs(ans))
}


vfun.risk <- function(scenario, options, risk) {
    ans <- switch(tolower(risk), mad = vfun.mad(scenario), ev = vfun.var(scenario),
...
simweights <- function(m, LB = 0, UB = 1, budget = 1, forecast = rep(0,
    m), constrain.positive = TRUE, rseed = NULL) {
    if (is.null(rseed))
    ...
    while (i == 0) {
        nb <- sum(bin)
        w <- rtruncnorm(nb, a = LB, b = UB, mean = (UB - LB)/2, sd = (UB -
            LB)/3)
        w <- budget * (w/sum(w))
        wx[as.logical(bin)] <- w
        if (constrain.positive) {
            if (sum(wx * forecast) >= 0)
                i <- 1
        } else {
            i <- 1
        }
