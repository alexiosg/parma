...
##
#################################################################################
.parmautility <- function(U = c("CARA", "Power"), method = c("moment",
    "scenario"), scenario = NULL, M1 = NULL, M2 = NULL, M3 = NULL, M4 = NULL,
    RA = 1, budget = 1, LB = rep(0, length(M1)), UB = rep(1, length(M1))) {
    if (tolower(method[1]) == "scemario") {
        ...
        if (length(UB) != m)
            UB <- rep(UB[1], m)
        sol <- switch(mx, c2 = try(nloptr(x0 = rep(1/m, m), eval_f = cara2.fn,
            eval_grad_f = cara2.gr, lb = LB, ub = UB, eval_g_eq = cara2.eq,
            eval_jac_g_eq = cara2.eqgr, opts = .nloptr.ctrl(), l = RA,
            M1 = M1, M2 = M2, budget = budget)), c4 = try(nloptr(x0 = rep(1/m,
            m), eval_f = cara4.fn, eval_grad_f = cara4.gr, lb = LB, ub = UB,
            eval_g_eq = cara4.eq, eval_jac_g_eq = cara4.eqgr, opts = .nloptr.ctrl(),
            l = RA, M1 = M1, M2 = M2, M3 = M3, M4 = M4, budget = budget),
            silent = TRUE))
        ...
        solution <- list(status = status, weights = weights, utility = utility,
            reward = reward, risk = NA)
    }
    ans <- new("parmaPort", solution = solution, model = model)
    return(ans)
}
...
parma4CARA <- function(M1, M2, M3, M4, RA = 1, budget = 1, LB, UB) {
    m <- length(M1)
    sol <- try(nloptr(x0 = rep(1/m, m), eval_f = cara4.fn, eval_grad_f = cara4.gr,
        lb = LB, ub = UB, eval_g_eq = cara4.eq, eval_jac_g_eq = cara4.eqgr,
        opts = .nloptr.ctrl(), l = RA, M1 = M1, M2 = M2, M3 = M3, M4 = M4,
        budget = budget), silent = TRUE)
    return(sol)
}

parma2CARA <- function(M1, M2, RA = 1, budget = 1, LB, UB) {
    m <- length(M1)
    sol <- try(nloptr(x0 = rep(1/m, m), eval_f = cara2.fn, eval_grad_f = cara2.gr,
        lb = LB, ub = UB, eval_g_eq = cara2.eq, eval_jac_g_eq = cara2.eqgr,
        opts = .nloptr.ctrl(), l = RA, M1 = M1, M2 = M2))
    return(sol)
}
...
cara2.gr <- function(w, l, M1, M2, budget) {
    ...
    dm3 <- 0
    dm4 <- 0
    g <- (l * exp(-l * pm1)) * (M1 %*% (1 + l2 * pm2 + l3 * pm3 + l4 *
        pm4)) - exp(-l * pm1) * (dl2 * dm2 + dl3 * dm3 + dl4 * dm4)
    return(as.numeric(-g))
}
