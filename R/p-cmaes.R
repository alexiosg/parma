...
eye <- function(n, m = n) {
    stopifnot(is.numeric(n), length(n) == 1, is.numeric(m), length(m) ==
        1)
    n <- floor(n)
    m <- floor(m)
    if (n <= 0 || m <= 0)
        return(matrix(NA, 0, 0)) else return(base::diag(1, n, m))
}
...
ones <- function(n, m = n) {
    ...
    m <- floor(m)
    if (n <= 0 || m <= 0)
        return(matrix(1, 0, 0)) else return(matrix(1, n, m))
}
...
zeros <- function(n, m = n) {
    ...
    m <- floor(m)
    if (n <= 0 || m <= 0)
        return(matrix(0, 0, 0)) else return(matrix(0, n, m))
}
...
repmat <- function(a, n, m = n) {
    ...
    if (!is.numeric(n) || !is.numeric(m) || length(n) != 1 || length(m) !=
        1)
...
cmaes.control <- function(options = list(StopFitness = -Inf, MaxFunEvals = Inf,
    MaxIter = "1e3*(N+5)^2/sqrt(popsize)", StopFunEvals = Inf, StopIter = Inf,
    TolX = "1e-11*max(insigma)", TolUpX = "1e3*max(insigma)", TolFun = 1e-12,
    TolHistFun = 1e-13, StopOnStagnation = TRUE, StopOnWarnings = TRUE,
    StopOnEqualFunctionValues = "2 + N/3", DiffMaxChange = Inf, DiffMinChange = 0,
    WarnOnEqualFunctionValues = FALSE, EvalParallel = FALSE, EvalInitialX = TRUE,
    Restarts = 0, IncPopSize = 2, PopSize = "4 + floor(3*log(N))", ParentNumber = "floor(popsize/2)",
    RecombinationWeights = c("superlinear", "linear", "constant"), DiagonalOnly = "0*(1+100*N/sqrt(popsize))+(N>=1000)",
    CMA = TRUE, Seed = "as.integer(Sys.time())", DispFinal = TRUE, DispModulo = 100,
    Warnings = FALSE), CMA = list(cs = "(mueff+2)/(N+mueff+3)", damps = "1 + 2*max(0,sqrt((mueff-1)/(N+1))-1) + cs",
    ccum = "(4 + mueff/N) / (N+4 + 2*mueff/N)", ccov1 = "2 / ((N+1.3)^2+mueff)",
    ccovmu = "2 * (mueff-2+1/mueff) / ((N+2)^2+mueff)", active = 0)) {

    xoptions <- list(StopFitness = -Inf, MaxFunEvals = Inf, MaxIter = "1e3*(N+5)^2/sqrt(popsize)",
        StopFunEvals = Inf, StopIter = Inf, TolX = "1e-11*max(insigma)",
        TolUpX = "1e3*max(insigma)", TolFun = 1e-12, TolHistFun = 1e-13,
        StopOnStagnation = TRUE, StopOnWarnings = TRUE, StopOnEqualFunctionValues = "2 + N/3",
        DiffMaxChange = Inf, DiffMinChange = 0, WarnOnEqualFunctionValues = FALSE,
        EvalParallel = FALSE, EvalInitialX = TRUE, Restarts = 0, IncPopSize = 2,
        PopSize = "4 + floor(3*log(N))", ParentNumber = "floor(popsize/2)",
        RecombinationWeights = c("superlinear"), DiagonalOnly = "0*(1+100*N/sqrt(popsize))+(N>=1000)",
        CMA = TRUE, Seed = "as.integer(Sys.time())", DispFinal = TRUE,
        DispModulo = 100, Warnings = FALSE)
    xCMA <- list(cs = "(mueff+2)/(N+mueff+3)", damps = "1 + 2*max(0,sqrt((mueff-1)/(N+1))-1) + cs",
        ccum = "4/(N+4)", ccov1 = "2 / ((N+1.3)^2+mueff)", ccovmu = "2 * (mueff-2+1/mueff) / ((N+2)^2+mueff)",
        active = 0)
    ...
    if (length(idx1) > 0)
        for (i in 1:length(idx1)) xoptions[[idx1[i]]] <- options[[i]][1]
    idx3 <- sapply(names(CMA), FUN = function(x) match(x, names(unlist(xCMA))))
    if (length(idx3) > 0)
        for (i in 1:length(idx3)) xCMA[[idx3[i]]] <- CMA[[i]][1]
...
myprctile <- function(inar, perc, idx = NULL) {
    ...
    for (p in perc) {
        if (p <= 100 * (0.5/N)) {
            ...
            res <- c(res, sar[i] + (sar[i + 1] - sar[i]) * (p - availablepercentiles[i])/(availablepercentiles[i +
                1] - availablepercentiles[i]))
        }
    }
    return(res)
}
local_noisemeasurement <- function(arf1, arf2, lamreev, theta, cutlimit = Inf) {
    ...
    if (any(diffarf1 == 0)) {
        ...
        warning(paste("\n", (sum(diffarf1 == 0)), " equal function values",
            sep = ""))
    }
    ...
    rankDelta <- matrix(ranks[1, ] - ranks[2, ] - sign(ranks[1, ] - ranks[2,
        ]), nrow = 1)
    # compute rank change limits using both ranks(1,...) and ranks(2,...)
    sumlim <- rep(0, lamreev)
    for (i in 1:lamreev) {
        sumlim[i] <- 0.5 * (myprctile(abs((1:(2 * lam - 1) - (ranks[1,
            i] - (ranks[1, i] > ranks[2, i])))), theta * 50) + myprctile(abs((1:(2 *
            lam - 1) - (ranks[2, i] - (ranks[2, i] > ranks[1, i])))), theta *
            50))
    }
    # compute measurement
...
cmaes <- function(pars, fun, lower = rep(0, length(pars)), upper = rep(1,
    length(pars)), insigma = 1, ctrl = cmaes.control(), ...) {
    ...
    while (irun <= .eval.control("Restarts", opts)) {
        ...
        } else if (!is.null(dim(as.matrix(xmean))) && dim(as.matrix(xmean))[2] >
            1) {
            xmean <- t(xmean)
            N <- length(xmean)
        } else {
        ...
        numberofvariables <- N
        lambda0 <- floor(.eval.control("PopSize", opts) * .eval.control("IncPopSize",
            opts)^(irun - 1))
        ...
        flgWarnOnEqualFunctionValues <- .eval.control("WarnOnEqualFunctionValues",
            opts)
        flgEvalParallel <- .eval.control("EvalParallel", opts)
        stopOnEqualFunctionValues <- .eval.control("StopOnEqualFunctionValues",
            opts)
        ...
        if (dim(as.matrix(xmean))[2] > 1 || dim(as.matrix(xmean))[1] !=
            N) {
            stop(paste("\nintial search point should be a vector of size ",
                N, sep = ""))
        }
        if (length(insigma) != 1 && length(insigma) != N) {
            stop(paste("\ninput parameter sigma should be a scalar or a vector of size ",
                N, sep = ""))
        }
        if (length(stopTolX) > 1 && length(stopTolX) != N) {
            stop(paste("\noption TolX should be a scalar or a vector of size ",
                N, sep = ""))
        }
        if (length(stopTolUpX) > 1 && length(stopTolUpX) != N) {
            stop(paste("\noption TolUpX should be a scalar or a vector of size ",
                N, sep = ""))
        }
        if (length(maxdx) > 1 && length(maxdx) != N) {
            stop(paste("\noption DiffMaxChange should be a scalar or a vector of size ",
                N, sep = ""))
        }
        if (length(mindx) > 1 && length(mindx) != N) {
            stop(paste("\noption DiffMinChange should be a scalar or a vector of size ",
                N, sep = ""))
        }
        if (length(lbounds) > 1 && length(lbounds) != N) {
            stop(paste("\noption lower should be a scalar or a vector of size ",
                N, sep = ""))
        }
        if (length(ubounds) > 1 && length(ubounds) != N) {
            stop(paste("\noption upper should be a scalar or a vector of size ",
                N, sep = ""))
        }
        # Initialize dynamic internal state parameters
        if (any(insigma <= 0))
            stop("\nInitial search volume (sigma) must be greater than zero")
        if (max(insigma)/min(insigma) > 1e+06)
        ...
        if (bnd$isactive) {
            ...
            ti <- tmp$idx
            # just in case
            if (length(ti) > 0 && any(ti) != 0 && Warnings)
            ...
            if (any(sigma * sqrt(diagC) > maxdx)) {
                ...
                if (Warnings)
                  warning(paste("Initial sigma multiplied by the factor ",
                    fac, ", because it was larger than half of one of the boundary intervals",
                    sep = ""))
            }
            idx <- as.numeric(lbounds > -Inf & ubounds < Inf)
            dd <- diagC
...
should be between 0.2 and 0.5 of the bounded interval in each coordinate. If all coordinates have lower and upper bounds sigma can be empty.")
        ...
        while (is.null(stopflag)) {
            ...
            if (countiter == 0 || lambda != lambda_last) {
                if (countiter > 0 && floor(log10(lambda)) != floor(log10(lambda_last)) &&
                  flgdisplay) {
                  cat(paste("\n  lambda = ", lambda, sep = ""))
                  lambda_hist <- cbind(lambda_hist, rbind(countiter + 1,
                    lambda))
                } else {
            ...
            if (countiter == 0 && flgdisplay) {
                if (mu == 1) {
                  ...
                  strw <- c(sprintf("%.0f", 100 * weights[1]), "  ", sprintf("%.0f",
                    100 * weights[-1]))
                } else {
                  strw <- c(sprintf("%.2g", 100 * weights[1:3]), "...",
                    tail(sprintf("%.2g", 100 * weights), 3))
                }
                cat("\nrun[", irun, "]  n=", N, ": (", mu, ",", lambda,
                  ")-CMA-ES(w=", "[", strw, "], mu_eff=", round(mueff,
                    2), ")\n")
                if (flgDiagonalOnly == 1) {
                  cat("\n    C is diagonal")
                } else if (flgDiagonalOnly) {
                  cat(paste("\n    C is diagonal for ", floor(flgDiagonalOnly),
                    " iterations", sep = ""))
                }
            }
            ...
            if (flgEvalParallel) {
                ...
                if (flgDiagonalOnly != 1) {
                  arx <- repmat(as.matrix(xmean), 1, lambda) + sigma *
                    (BD %*% arz)
                  # Eq. (1)
                } else {
                  # check
                  arx <- repmat(as.matrix(xmean), 1, lambda) + repmat(as.matrix(sigma *
                    diagD), 1, lambda) * arz
                }
                #if(noiseHandling){
                #\tif(noiseEpsilon == 0){
            ...
            if (length(hn) > 0) {
                ...
                if (is.null(arxvalid))
                  arxvalid <- matrix(NA, ncol = length(hn), nrow = N)
                if (is.null(arz))
                  arz <- matrix(NA, ncol = length(hn), nrow = N)
                if (is.null(arx))
                  arx <- matrix(NA, ncol = length(hn), nrow = N)
                ...
                if (dim(arx)[2] < length(hn)) {
                  ...
                  arxvalid <- cbind(arxvalid, matrix(NA, ncol = diffl,
                    nrow = N))
                }
                for (k in hn) {
                  # fitness.raw(k) = NaN; 
                  ...
                  while (is.na(fitness$raw[k])) {
                    if (k <= lambda) {
                      ...
                      if (flgDiagonalOnly) {
                        arx[, k] <- arx[, k - lambda] + (noiseEpsilon *
                          sigma) * diagD * rnorm(N)
                      } else {
                        arx[, k] <- arx[, k - lambda] + (noiseEpsilon *
                          sigma) * (BD %*% rnorm(N))
                    ...
                    if (tries%%100 == 0) {
                      if (Warnings)
                        warning(paste("\n", tries, " NA objective function values at evaluation ",
                          counteval, sep = ""))
                    }
                  }
                  counteval <- counteval + 1
            ...
            if (1 < 3 && bnd$isactive) {
                ...
                if (bnd$iniphase) {
                  if (length(ti) > 0 && any(ti) != 0) {
                    bnd$weights[which(bnd$isbounded == 0)] <- 2.0002 *
                      median(bnd$dfithist)
                    if (bnd$flgscale == 0) {
                ...
                bnd$arpenalty <- as.numeric((bnd$weights/bnd$scale) %*%
                  (arxvalid - arx)^2)
            ...
            # minimization
            fitness$hist[-1] <- fitness$hist[1:(length(fitness$hist) -
                1)]
            # record short history of
            fitness$hist[1] <- fitness$raw[1]
            # best fitness values
            if (length(fitness$histbest) < (120 + ceiling(30 * N/lambda)) ||
                (countiter%%5 == 0 && length(fitness$histbest) < 20000)) {
                ...
                fitness$histbest[-1] <- fitness$histbest[1:(length(fitness$histbest) -
                  1)]
                fitness$histmedian[-1] <- fitness$histmedian[1:(length(fitness$histmedian) -
                  1)]
            ...
            fitness$histsel[-1] <- fitness$histsel[1:(length(fitness$histsel) -
                1)]
            ...
            ps <- as.numeric((1 - cs) * ps + sqrt(cs * (2 - cs) * mueff) *
                (B %*% zmean))
            # Eq. (4)
            hsig <- as.integer(norm(as.matrix(ps), type = "F")/sqrt(1 -
                (1 - cs)^(2 * countiter))/chiN < (1.4 + 2/(N + 1)))
            ...
            #  hsig = 1;\t
            pc <- (1 - cc) * pc + hsig * (sqrt(cc * (2 - cc) * mueff)/sigma) *
                (xmean - xold)
            # Eq. (2)
            #if(hsig == 0){
            ...
            if ((ccov1 + ccovmu) > 0) {
                ...
                if (flgDiagonalOnly) {
                  ...
                  #  % plus rank mu update
                  diagC <- (1 - ccov1_sep - ccovmu_sep + (1 - hsig) * ccov1_sep *
                    cc * (2 - cc)) * diagC + ccov1_sep * pc^2 + ccovmu_sep *
                    (diagC * (arz[, fitness$idxsel[1:mu]]^2 %*% weights))
                  #   * (repmat(diagC,1,mu) .* arz(:,fitness.idxsel(1:mu)).^2 * weights);
                  diagD <- sqrt(diagC)
                  # % replaces eig(C)
                } else {
                  arpos <- (arx[, fitness$idxsel[1:mu]] - repmat(as.matrix(xold),
                    1, mu))/sigma
                  ...
                  if (flgActiveCMA > 0) {
                    ...
                    neg$ccov <- (1 - ccovmu) * 0.25 * neg$mueff/((N + 2)^1.5 +
                      2 * neg$mueff)
                    ...
                    arzneg <- arz[, fitness$idxsel[seq(lambda, lambda -
                      neg$mu + 1, by = -1)]]
                    ...
                    # for the record
                    if (flgActiveCMA < 2) {
                      arzneg <- arzneg * repmat(t(arnormfacs[idxnorms]),
                        N, 1)
                      #  E x*x' is N
                    ...
                    if (flgActiveCMA%%1 == 1 && (1 - neg$ccov * arnorms[idxnorms]^2 %*%
                      weights) < neg$minresidualvariance) {
                      ...
                      neg$ccovfinal <- min(neg$ccov, (1 - ccovmu) * (1 -
                        neg$minresidualvariance)/maxeigenval)
                      # -ccov1 removed to avoid error message??
                      if (neg$ccovfinal < neg$ccov) {
                        if (flgdisplay)
                          cat(paste("\nactive CMA at iteration ", countiter,
                            " : max EV == ", sprintf("%0.3e", maxeigenval),
                            " ", sprintf("%0.3e", neg$ccov), " ", sprintf("%0.3e",
                              neg$ccovfinal), sep = ""))
                      }
                    }
                    # xmean = xold;  # the distribution does not degenerate!? 
                    # update C
                    C <- (1 - ccov1 - ccovmu + neg$alphaold * neg$ccovfinal +
                      (1 - hsig) * ccov1 * cc * (2 - cc)) * C + ccov1 *
                      pc %*% t(pc) + (ccovmu + (1 - neg$alphaold) * neg$ccovfinal) *
                      arpos %*% (repmat(as.matrix(weights), 1, N) * t(arpos)) -
                      neg$ccovfinal * Cneg
                    # minus rank mu update
                  } else {
                    # no active (negative) update
                    C <- (1 - ccov1 - ccovmu + (1 - hsig) * ccov1 * cc *
                      (2 - cc)) * C + ccov1 * pc %*% t(pc) + ccovmu * arpos %*%
                      (repmat(as.matrix(weights), 1, N) * t(arpos))
            ...
            if (11 < 2 && !flgscience) {
                ...
                if (sum(ps^2)/N > (1.5 + 10 * (2/N)^5) && fitness$histsel[1] >
                  max(fitness$histsel[2:3])) {
                  ps <- ps * sqrt(N * (1 + max(0, log(sum(ps^2)/N)))/sum(ps^2))
                  if (flgdisplay) {
                    cat(paste("\nMomentum in ps removed at [niter neval]=[",
                      countiter, " ", counteval, "]", sep = ""))
                  }
                }
            }
            # Adapt sigma
            if (flg_future_setting) {
                ...
                # exp(1) is still not reasonably small enough
                sigma <- sigma * exp(min(1, (sqrt(sum(ps^2))/chiN - 1) *
                  cs/damps))
                # Eq. (5)
            }
            ...
            if (!flgDiagonalOnly && (ccov1 + ccovmu + neg$ccov) > 0 &&
                (countiter%%(1/(ccov1 + ccovmu + neg$ccov)/N/10)) < 1) {
                C <- triu(C) + t(triu(C, 1))
                ...
                if (any(!is.finite(diagD))) {
                  stop(paste("\nfunction eigen returned non-finited eigenvalues, cond(C)=",
                    cond(C), sep = ""))
                }
                if (any(!is.finite(as.vector(B)))) {
                  stop(paste("\nfunction eigen returned non-finited eigenvectors, cond(C)=",
                    cond(C), sep = ""))
                }
                ...
                if (min(diagD) <= 0) {
                  if (stopOnWarnings) {
                    ...
                    if (Warnings)
                      warning(paste("\nIteration ", countiter, " :Eigenvalue (smaller) than zero",
                        sep = ""))
                    diagD[diagD < 0] <- 0
                    tmp <- max(diagD)/1e+14
                    C <- C + tmp * eye(N, N)
                ...
                if (max(diagD) > 1e+14 * min(diagD)) {
                  if (stopOnWarnings) {
                    stopflag <- c(stopflag, "warnconditioncov")
                  } else {
                    if (Warnings)
                      warning(paste("\nIteration ", countiter, " :condition of C at upper limit",
                        sep = ""))
                    tmp <- max(diagD)/1e+14 - min(diagD)
                    C <- C + tmp * eye(N, N)
                    diagD <- diagD + tmp * ones(N, 1)
                  }
                }
            ...
            if (any(xmean == (xmean + 0.2 * sigma * sqrt(diagC)))) {
                if (stopOnWarnings) {
                  ...
                  if (Warnings)
                    warning(paste("\nIteration ", countiter, ": coordinate axis std deviation too low",
                      sep = ""))
                  if (flgDiagonalOnly) {
                    diagC <- diagC + (ccov1_sep + ccovmu_sep) * (diagC *
                      (xmean == xmean + 0.2 * sigma * sqrt(diagC)))
                  } else {
                    C <- C + (ccov1 + ccovmu) * diag(diagC * (xmean ==
                      xmean + 0.2 * sigma * sqrt(diagC)))
                  }
            ...
            if (all(xmean == (xmean + tmp))) {
                ...
                if (stopOnWarnings) {
                  ...
                  warning(paste("\nIteration ", countiter, ": main axis standard deviation ",
                    sigma * diagD[i], " has no effect", sep = ""))
            ...
            if (fitness$sel[1] == fitness$sel[1 + ceiling(0.1 + lambda/4)]) {
                ...
                if (stopOnEqualFunctionValues == 1) {
                  ...
                  arrEqualFunvals <- c(countiter, arrEqualFunvals[1:(nx -
                    1)])
                  ...
                  if (flgWarnOnEqualFunctionValues) {
                    if (Warnings)
                      warning(paste("\nIteration ", countiter, ": equal function values f=",
            ...
            if (countiter > 2 && myrange(c(fitness$hist, fitness$sel[1])) ==
                0) {
                if (stopOnWarnings) {
                  stopflag <- c(stopflag, "warnequalfunvalhist")
                } else {
                  if (Warnings)
                    warning(paste("\nIteration ", countiter, ": equal function values in history at maximal main axis sigma ",
                      sigma * max(diagD), sep = ""))
                }
                sigma <- sigma * exp(0.2 + cs/damps)
            }
            ...
            out$solutions$recentbest$evals <- counteval + fitness$idx[1] -
                lambda
            ...
            out$solutions$recentworst$evals <- counteval + fitness$idx[length(fitness$idx)] -
                lambda
            if (fitness$hist[1] < out$solutions$bestever$f) {
                out$solutions$bestever$x <- arxvalid[, fitness$idx[1]]
                out$solutions$bestever$f <- fitness$hist[1]
                out$solutions$bestever$evals <- counteval + fitness$idx[1] -
                  lambda
                bestever <- out$solutions$bestever
            }
            # Set stop flag
            ...
            if (countiter > 2 && myrange(c(fitness$sel, fitness$hist)) <=
                stopTolFun) {
                stopflag <- c(stopflag, "tolfun")
            }
            if (countiter >= length(fitness$hist) && myrange(fitness$hist) <=
                stopTolHistFun) {
                stopflag <- c(stopflag, "tolhistfun")
            }
            l <- floor(length(fitness$histbest)/3)
            nl <- length(fitness$histmedian)
            if (1 < 2 && stopOnStagnation && countiter > (N * (5 + 100/lambda)) &&
                length(fitness$histbest) > 100 && median(fitness$histmedian[1:l]) >=
                median(fitness$histmedian[(nl - l):nl]) && median(fitness$histbest[1:l]) >=
                median(fitness$histbest[(nl - l):nl])) {
            ...
            if (verbosemodulo > 0 && is.finite(verbosemodulo)) {
                if (countiter == 1 || countiter%%(10 * verbosemodulo) <
                  1) {
                  #cat('\\nIter, #f:   f-value\\t  (median,worst)\\t |Axis Ratio|\\t idx:Min SD idx:Max SD')
                  cat("\n Iter        #f:           f-value")
                }
                if (countiter%%verbosemodulo < 1 || (verbosemodulo > 0 &&
                  is.finite(verbosemodulo) && (countiter < 3 || !is.null(stopflag)))) {
                  ...
                  cat(c("\n", countiter, rep("", max(1, 10 - floor(log10(countiter)))),
                    counteval, rep("", max(1, 10 - floor(log10(counteval)))),
                    sprintf("%.5e", fitness$hist[1])))
                }
                ...
                if (countiter < 3) {
                  ...
                  timer$c <- max(1e-05, 0.1/sqrt(countiter))
                  # mean over all or 1e-5
                }
                # get average time per iteration
                timer$t1 <- Sys.time()
                timer$act <- max(0, timer$t1 - timer$t0)
                timer$nonoutput <- (1 - timer$c) * timer$nonoutput + timer$c *
                  timer$act
                ...
                timer$t2 <- Sys.time()
                timer$recording <- timer$recording + timer$c * max(0, timer$t2 -
                  timer$t1)
                # plot
                timer$t3 <- Sys.time()
                if (!is.null(stopflag) || timer$saving < (0.05 * timer$nonoutput) ||
                  countiter == 100) {
                  xmin <- arxvalid[, fitness$idx[1]]
                  fmin <- fitness$raw[1]
                  timer$saving <- timer$saving + timer$c * max(0, Sys.time() -
                    timer$t3)
                }
            }
            timer$t0 <- Sys.time()
    ...
    cat("\n")
    return(list(par = xmin, objective = fmin, out = out, counteval = counteval,
        stopflag = stopflag, bestever = bestever))
}

################################################################################
# Test Functions
frosenbrock <- function(x) {
    f <- 100 * sum((x[1:(length(x) - 1)]^2 - x[-1])^2) + sum((x[1:(length(x) -
        1)] - 1)^2)
    return(f)
}
...
fcigar <- function(x) {
    return(x[1]^2 + 1e+06 * sum(x[-1]^2))
}

fcigtab <- function(x) {
    return(x[1]^2 + 1e+08 * x[length(x)]^2 + 10000 * sum(x[2:(length(x) -
        1)]^2))
}

ftablet <- function(x) {
    return(1e+06 * x[1]^2 + sum(x[-1]^2))
}

felli <- function(x) {
    N <- length(x)
    f <- sum(1e+06^((0:(N - 1))/(N - 1)) * x^2)
    return(f)
}

felli100 <- function(x) {
    N <- length(x)
    f <- sum(10000^((0:(N - 1))/(N - 1)) * x^2)
    return(f)
}
...
ftwoaxes <- function(x) {
    ...
    f <- sum(x[1:floor(N/2)]^2) + 1e+06 * sum(x[floor(1 + N/2):N]^2)
    return(f)
}
