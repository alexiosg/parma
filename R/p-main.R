...
.parmaspec <- function(scenario = NULL, probability = NULL, S = NULL, Q = NULL,
    ...
    targetType = c("inequality", "equality"), risk = c("MAD", "MiniMax",
        "CVaR", "CDaR", "EV", "LPM", "LPMUPM"), riskType = c("minrisk",
        "optimal", "maxreward"), riskB = NULL, options = list(alpha = 0.05,
        threshold = 999, moment = 1, lmoment = 1, umoment = 1, lthreshold = -0.01,
        uthreshold = 0.01), LB = NULL, UB = NULL, budget = 1, leverage = NULL,
    ineqfun = NULL, ineqgrad = NULL, eqfun = NULL, eqgrad = NULL, uservars = list(),
    ineq.mat = NULL, ineq.LB = NULL, ineq.UB = NULL, eq.mat = NULL, eqB = NULL,
    max.pos = NULL, asset.names = NULL, ...) {
    ...
    ### a bunch of checks
    indx <- rep(0, 8)
    names(indx) <- c("datatype", "benchmark", "targettype", "risk", "risktype",
        "leverage", "aux1", "aux2")
    ...
    if (!is.null(scenario)) {
        ...
        indx[3] <- tmp
        tmp <- match(tolower(risk[1]), tolower(c("MAD", "MiniMax", "CVaR",
            "CDaR", "EV", "LPM", "LPMUPM")))
        if (is.na(tmp))
    ...
    modeldata <- list(scenario = scenario, probability = probability, S = S,
        Q = Q, qB = qB, benchmark = benchmark, benchmarkS = benchmarkS,
        forecast = forecast, target = target, riskB = riskB, asset.names = asset.names,
        uservars = uservars)

    constraints <- list(LB = LB, UB = UB, budget = budget, leverage = leverage,
        ineqfun = ineqfun, ineqgrad = ineqgrad, eqfun = eqfun, eqgrad = eqgrad,
        ineq.mat = ineq.mat, ineq.LB = ineq.LB, ineq.UB = ineq.UB, eq.mat = eq.mat,
        eqB = eqB, uservars = uservars, max.pos = max.pos)

    ans <- new("parmaSpec", model = model, modeldata = modeldata, constraints = constraints)
    return(ans)
}
...
.spec2minNLP <- function(spec) {
    ...
    if (spec@model$indx[2] == 0) {
        optvars$Data <- switch(tolower(spec@model$risk), mad = scale(spec@modeldata$scenario,
            scale = FALSE), ev = scale(spec@modeldata$scenario, scale = FALSE),
            minimax = spec@modeldata$scenario, cvar = spec@modeldata$scenario,
            cdar = spec@modeldata$scenario, lpm = spec@modeldata$scenario,
    ...
    if (tolower(risk) == "cvar") {
        optvars$fm <- m + 1
        x0 <- as.numeric(c(quantile(optvars$Data %*% rep(1/m, m), spec@model$options$alpha),
            rep(1/m, m)))
        ...
        optvars$fm <- m + 1
        x0 <- as.numeric(c(-min(optvars$Data %*% rep(1/m, m)), rep(1/m,
            m)))
...
.spec2optNLP <- function(spec, pcontrol = list(ubounds = 10000, mbounds = 1e+05,
    penalty = 10000, startp = 150)) {
    optvars <- list()
    optvars$widx <- spec@model$widx
    optvars$midx <- spec@model$midx
    optvars$vidx <- spec@model$vidx
    ...
    if (tolower(risk) == "lpmupm") {
        if (spec@model$options$lthreshold == 999 || spec@model$options$uthreshold ==
            999) {
            optvars$Data <- scale(optvars$Data, scale = FALSE)
            ...
            if (optvars$lthreshold > optvars$uthreshold)
    ...
    if (tolower(risk) == "cvar") {
        optvars$fm <- m + 2
        x0 <- as.numeric(c(5 * quantile(optvars$Data %*% rep(1/m, m), spec@model$options$alpha),
            5 * rep(1/m, m), 5))
        optvars$fLB <- c(-pcontrol$ubounds, rep(-pcontrol$ubounds, m),
            1e-08)
        ...
        optvars$fm <- m + 1
        x0 <- c(rep(1/m, m), 2)
        optvars$fLB <- c(rep(-pcontrol$ubounds, m), 1e-08)
...
.spec2minGNLP <- function(spec, pcontrol = list(ubounds = 10000, mbounds = 1e+05,
    penalty = 10000, startp = 150)) {
    optvars <- list()
    optvars$widx <- spec@model$widx
    optvars$midx <- spec@model$midx
...
.spec2optGNLP <- function(spec, pcontrol = list(ubounds = 10000, mbounds = 1e+05,
    penalty = 10000, startp = 150)) {
    optvars <- list()
    optvars$widx <- spec@model$widx
    optvars$midx <- spec@model$midx
...
.parmasolve <- function(spec, type = NULL, solver = NULL, solver.control = list(),
    ...
    if (any(is.na(mm))) {
        ...
        warning(paste(c("unidentified option(s) in parma.control:\n", enx),
            sep = "", collapse = " "), call. = FALSE, domain = NULL)
    }
    if (is.null(parma.control$ubounds))
        parma.control$ubounds <- 10000 else parma.control$ubounds <- parma.control$ubounds[1]
    if (is.null(parma.control$mbounds))
        parma.control$mbounds <- 1e+05 else parma.control$mbounds <- parma.control$mbounds[1]
    if (is.null(parma.control$penalty))
        parma.control$penalty <- 10000 else parma.control$penalty <- parma.control$penalty[1]
    if (is.null(parma.control$eqSlack))
        parma.control$eqSlack <- 1e-05 else parma.control$eqSlack <- parma.control$eqSlack[1]

    available.problems <- toupper(c("LP", "MILP", "QP", "MIQP", "SOCP",
        "NLP", "MINLP", "GNLP")[which(spec@model$type == 1)])
    ...
    if (spec@model$indx[5] == 1) {
        optvars <- switch(tolower(type), lp = .spec2LP(spec), milp = .spec2LP(spec),
            nlp = .spec2minNLP(spec), qp = .spec2QP(spec), socp = .spec2SOCP(spec),
            gnlp = .spec2minGNLP(spec, parma.control))
    } else {
        optvars <- switch(tolower(type), lp = .spec2LP(spec), milp = .spec2LP(spec),
            nlp = .spec2optNLP(spec, parma.control), qp = .spec2QP(spec),
            socp = .spec2SOCP(spec), gnlp = .spec2optGNLP(spec, parma.control))
    }
    uservars <- spec@modeldata$uservars
    if (!is.null(x0)) {
        if (length(optvars$x0) != length(x0))
            stop("\nparma: wrong length for x0!")
        optvars$x0 <- x0
    }
    if (!is.null(w0)) {
        if (length(optvars$widx) != length(w0))
            stop("\nparma: wrong length for w0!")
        optvars$x0[optvars$widx] <- w0
    }
    if (type == "LP" && is.null(solver))
        solver <- "GLPK"
    sol <- switch(toupper(type), LP = lpport(optvars, solver, ...), MILP = milpport(optvars,
        solver, ...), NLP = nlpport(optvars, uservars, control = solver.control,
        ...), QP = qpport(optvars, ...), SOCP = socpport(optvars, control = solver.control,
        eqSlack = parma.control$eqSlack, ...), GNLP = gnlpport(optvars,
        uservars, solver = solver, control = solver.control, ...))
    # arbitrage check
    if (type != "QP" & type != "SOCP") {
        arbitrage <- .arbcheck(sol$weights, spec@modeldata$scenario, spec@model$options,
            spec@model$risk)
    } else {
        arbitrage <- c(0, 0)
    }
    ...
    ret <- new("parmaPort", solution = sol, model = spec@model)
    return(ret)
}
################################################################################
.parmafrontier <- function(spec, n.points = 100, miny = NULL, maxy = NULL,
    type = NULL, solver = NULL, solver.control = list(), parma.control = list(ubounds = 10000,
    ...
    if (!is.null(spec@modeldata$S)) {
        ...
        ans <- s.parmafrontier(spec, n.points = n.points, miny = miny,
            maxy = maxy, type = type, solver = solver, solver.control = solver.control,
            parma.control = parma.control, cluster = cluster)
    }
    return(ans)
}

s.parmafrontier <- function(spec, n.points = 100, miny = NULL, maxy = NULL,
    type = NULL, solver = NULL, solver.control = list(), parma.control = list(ubounds = 10000,
        mbounds = 1e+05, penalty = 10000), cluster = NULL) {
    ...
    if (is.null(miny)) {
        ...
        solx <- try(parmasolve(xspec, type = type, solver = solver, solver.control = solver.control,
            parma.control = parma.control), silent = TRUE)
        if (!inherits(solx, "try-error"))
            minb <- parmareward(solx)
    } else {
        minb <- miny
    }
    if (!is.null(maxy)) {
    ...
    if (!is.null(cluster)) {
        ...
        clusterExport(cluster, c("fs", "spec", "type", "solver", "solver.control",
            "parma.control"), envir = environment())
        sol <- parLapply(cluster, 1:n.points, fun = function(i) {
            xspec <- spec
            parmaset(xspec) <- list(target = fs[i])
            tmp <- parmasolve(xspec, type = type, solver = solver, solver.control = solver.control,
...
m.parmafrontier <- function(spec, n.points = 100, type = "QP", solver.control = list(abs.tol = 1e-08,
    rel.tol = 1e-08, Nu = 2, max.iter = 5250, BigM.K = 4, BigM.iter = 15),
    miny = NULL, maxy = NULL, cluster = NULL) {
    targettype <- parmaget(spec, "targetType")
    risktype <- parmaget(spec, "riskType")
    if (risktype != "minrisk")
        stop("\nspec riskType must be minrisk...fix and resubmit.")
    ...
    if (!is.null(cluster)) {
        clusterEvalQ(cluster, require(parma))
        clusterExport(cluster, c("fs", "spec", "type", "solver.control"),
            envir = environment())
        sol <- parLapply(cluster, 1:n.points, fun = function(i) {
            xspec <- spec
            parmaset(xspec) <- list(target = fs[i])
...
.checkconsfun <- function(fun, name = "ineqfun") {
    ...
    for (i in 1:n) {
        if (!is.function(fun[[i]]))
            stop(paste("\n", name, " list constains non functions at position: ",
                i, sep = ""))
    }
    return(0)
}
